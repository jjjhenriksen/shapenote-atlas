#!/usr/bin/env python3
"""Reconcile Eternal Ages' retained shape draft into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/476-eternal-ages/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/476-eternal-ages-7208bbd8b1.jpg"
RAW_OMR = ROOT / "work/omr/476-eternal-ages/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-476-eternal-ages-7208bbd8b1/work__source-images__2025__476-eternal-ages-7208bbd8b1.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/476-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/476-eternal-ages-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/476-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    for part in children(root, "part"):
        ...
        # The scan prints E Major; E is four sharps.
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/476",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies ETERNAL AGES. L.M., E major, 4/4, Isaac Watts (1707), Leah Velleman (2019), four vocal parts, source-visible lyrics, repeat bars with numbered first/second endings, and a terminal double bar; a diagonal DO NOT COPY watermark intersects the middle systems.",
        "Audiveris reports 20 raw measures (10 in each system), while the retained source MXL and normalized-v2 export 18 measures per part. The normalized-v2 working score contains 168 pitched events, 14 empty exported measures, and 64 of 72 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 168 four-shape noteheads are derived from OMR pitch steps plus the observed E-major key, not independently verified against every printed notehead. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "The Easton and Halebourne same-title witnesses are alternate settings with different titles/credits and musical material; their existing rejection audits remain separate and they were not used to fill the 2025 score. No authorized exact-edition structured Eternal Ages witness was available.",
    ]
    audit = {
        "queueId": "sh2025/476", "edition": "Sacred Harp, 2025 Edition", "songNo": "476", "title": "Eternal Ages",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=476", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/476-Eternal-Ages/476.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "ETERNAL AGES. L.M.", "composer": "Isaac Watts, 1707", "arranger": "Leah Velleman, 2019", "key": "E major", "mode": "major", "timeSignature": "4/4", "meter": "Long Meter (8,8,8,8)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 20, "sourceRawMeasuresBySystem": [10, 10], "exportedMeasuresByPart": {"P1": 18, "P2": 18, "P3": 18, "P4": 18},
        ...
        "candidateWitness": {"available": True, "candidateRole": "Alternate same-title settings are retained separately and rejected as source mismatches; no authorized exact-edition structured Eternal Ages witness was available.", "candidates": [
            {"title": "Easton", "status": "rejected-source-mismatch", "audit": "work/source-transcriptions/2025/476-eternal-ages-easton-comparison.json"},
            {"title": "Halebourne", "status": "rejected-source-mismatch", "audit": "work/source-transcriptions/2025/476-eternal-ages-halebourne-comparison.json"},
        ]},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromPriorReviewDraft": True, "status": "review-only-not-source-verified", "corrections": ["source-observed E-major mode", "source-observed 4/4 meter", "explicit autonomous-block metadata", "preserved derived four-shape tags without claiming per-note verification", "lyrics and repeat/ending semantics intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": source_hash, "retainedImagePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedImageSha256": retained_hash, "rawOmrSha256": raw_hash, "workingOmrSha256": working_hash, "alternateWitnessesInspected": ["Easton", "Halebourne"], "method": "full-resolution visual inspection of immutable scan plus structural/event/duration audit of raw and normalized-v2 OMR; alternate settings retained separately", "blockingFindings": blocking},
        "blockingFindings": blocking,
        "blockingReason": "Autonomous promotion is blocked by the 20-versus-18 measure topology discrepancy, 64-of-72 normalized-v2 4/4 duration failures, 14 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
