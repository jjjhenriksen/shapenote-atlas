#!/usr/bin/env python3
"""Reconcile Mournful Joy's retained scan OMR into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/562-mournful-joy/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/562-mournful-joy-46f7ed1f29.jpg"
RAW_OMR = ROOT / "work/omr/562-mournful-joy/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/562-mournful-joy-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/562-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, raw_hash: str) -> bytes:
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/562",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies MOURNFUL JOY. C.M.D., E minor, 4/4, John Newton (1779), William Cleary (2018), four vocal parts, and source-visible lyrics. Repeat bars with numbered first/second endings and a terminal double bar are visible; a diagonal DO NOT COPY watermark intersects the middle/lower systems.",
        "Audiveris reports 26 raw measures across three systems (10, 7, and 9), while the retained MXL exports 24 measures per part. The retained score contains 242 pitched events, 8 empty exported measures, and 73 of 96 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        ...
        "No authorized exact-edition structured Mournful Joy witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, endings, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/562", "edition": "Sacred Harp, 2025 Edition", "songNo": "562", "title": "Mournful Joy",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=562", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/562-Mournful-Joy/562.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "MOURNFUL JOY. C.M.D.", "composer": "John Newton, 1779", "arranger": "William Cleary, 2018", "key": "E minor", "mode": "minor", "timeSignature": "4/4", "meter": "Common Meter Double (8,6,8,6,8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 26, "sourceRawMeasuresBySystem": [10, 7, 9], "exportedMeasuresByPart": {"P1": 24, "P2": 24, "P3": 24, "P4": 24},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Mournful Joy witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromRetainedOmr": True, "status": "review-only-not-source-verified", "corrections": ["source-observed E-minor key", "source-observed 4/4 meter", "explicit autonomous-block metadata", "lyrics/repeats/endings/shapes intentionally not fabricated"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 26-versus-24 measure topology discrepancy, 73-of-96 retained-OMR 4/4 duration failures, 8 empty exported measures, absent lyrics/repeat/ending semantics/per-note shape encoding, watermark-intersected notation, differing source-image hashes, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
