#!/usr/bin/env python3
"""Create a source-derived, fail-closed Rest for the Weary derivative."""
...
SOURCE = ROOT / "work/omr/154-rest-for-the-weary/source.mxl"
SOURCE_IMAGE = ROOT / "work/omr/154-rest-for-the-weary/source.jpg"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/154-rest-for-the-weary-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/154-rest-for-the-weary-source-correction-v2-comparison.json"
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    fields = {
        ...
        "atlas-source-meter": "8s & 7s",
        "atlas-source-repeat-ending": "Repeat/barline structure and repeated refrain are visible on the immutable page; retained OMR only contains initial/terminal regular bars and does not encode a complete source structure",
        ...
        "atlas-blocker": "The source page visibly prints REST FOR THE WEARY. 8s & 7s., E-flat Major, Robert Robinson 1758, four vocal parts, lyrics, repeated refrain, and repeat/barline structure. Retained OMR exports 13 measures per part while Audiveris logged 16 raw measures (7 in system 1 and 9 in system 2); it has sparse events, no usable divisions, no lyrics, no shapes, and no complete structural encoding. No notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly prints REST FOR THE WEARY. 8s & 7s., E-flat Major, Robert Robinson 1758, four vocal parts, lyric underlay, repeated refrain, and repeat/barline structure.",
        "The retained source OMR exports 13 measure elements per part with event counts P1=56, P2=11, P3=55, P4=55, including one rest and 18 empty measures; Audiveris reports 16 raw measures (7 in system 1 and 9 in system 2), so complete source topology and event coverage are not established.",
        ...
        "The retained OMR has no lyrics, four-shape tags, or complete repeat/refrain semantics. The derivative adds source metadata and derived shapes without rewriting uncertain events.",
        "No authorized same-title structured candidate exists, so no alternate tune or edition was used to fill missing notation.",
        "No separate retained work/source-images/2025 duplicate matching 154 Rest for the Weary exists; the immutable work/omr/154-rest-for-the-weary/source.jpg is the canonical local visual witness.",
    ]
    audit = {
        "queueId": "sh2025/154",
        ...
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=154",
            ...
            "immutable": True,
            "directObservations": {
                "header": "REST FOR THE WEARY. 8s & 7s.",
                ...
                "sourceMeasureCountStatus": "Audiveris direct page analysis reports 16 raw measures (7 in system 1 and 9 in system 2); exported OMR contains 13 per part and does not establish complete source topology",
            },
            "retainedSourceImageMissing": {"expectedGlob": "work/source-images/2025/*154*rest*weary*.jpg", "status": "not-found"},
        },
        "inputOmr": {"path": "work/omr/154-rest-for-the-weary/source.mxl", "sha256": source_hash, "status": "retained-source-scan-omr"},
        "candidateWitness": {"status": "none-authorized", "sameTitleStructuredCandidate": False},
        "correctedDraft": {
            "path": "work/omr/autonomous-transcriptions/2025/154-rest-for-the-weary-source-correction-v2.mxl",
            "sha256": draft_hash,
            "summary": summary,
            ...
            "corrections": [
                ...
                "source repeat/refrain and lyric visibility recorded in provenance",
    ...
    print(json.dumps({"record": "sh2025/154", "status": audit["comparisonStatus"], "sourceImageSha256": image_hash, "inputOmrSha256": source_hash, "correctedDraftSha256": draft_hash, "summary": summary, "audit": str(AUDIT.relative_to(ROOT))}, ensure_ascii=False, sort_keys=True))
    return 0
