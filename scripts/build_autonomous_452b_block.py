#!/usr/bin/env python3
"""Create a fail-closed, source-derived Martin OMR derivative."""
...
SOURCE = ROOT / "work" / "omr" / "452b-martin" / "source.mxl"
IMAGE = ROOT / "work" / "omr" / "452b-martin" / "source.jpg"
OUTPUT = ROOT / "work" / "omr" / "autonomous-transcriptions" / "2025" / "452b-autonomous-blocked.mxl"
AUDIT = ROOT / "work" / "source-transcriptions" / "2025" / "452b-martin-autonomous-comparison.json"

# Martin is printed in F major; spell the retained pitches in that key.
    ...
    with zipfile.ZipFile(SOURCE) as archive:
        ...
        for part in parts:
            ...
            for note in [item for item in part.iter() if ln(item.tag) == "note"]:
                ...
                head.text = SHAPES[step.text.strip().upper()]
        ...
        fields = {
            ...
            "atlas-blocker": f"The source visibly prints MARTIN. 7s D., F major, 6/4, four vocal parts, and seven source-aligned measures. Retained OMR instead records 3/4 with divisions=2; its cursor duration fails against the retained 3/4 grouping at {failure_text}. P1 m3,m5,m7, P2 m1, P3 m1,m3,m5, and P4 m1,m4,m5,m6,m7 are blank while the source shows notation. OMR key and mode metadata are absent in every part. The diagonal watermark intersects middle-system events; only those intersected events remain unresolved for that reason.",
        }
...
def main() -> int:
    ...
    blocking = [
        "The retained source scan visibly prints MARTIN. 7s D., F major, 6/4, four vocal parts, and seven source-aligned measures.",
        "The retained OMR uses 3/4 rather than the source-visible 6/4 and has the named cursor-duration failures; multiple source-visible groups are blank in the OMR.",
        "OMR key and mode metadata are absent in every part; the derivative follows source-visible F major without silently rewriting event pitches.",
    ...
    audit = {
        ...
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=452b",
            "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/452b-Martin/452b.jpg",
            "sourceImagePath": "work/omr/452b-martin/source.jpg",
        ...
        "inputOmr": {"path": "work/omr/452b-martin/source.mxl", "sha256": source_hash, "status": "retained-source-scan-omr"},
        "correctedDraft": {
            "path": "work/omr/autonomous-transcriptions/2025/452b-autonomous-blocked.mxl",
            "sha256": output_hash,
            "summary": summary,
            "corrections": ["four part structure preserved", "source F-major key/mode", "source 6/4 time signature", "derived four-shape notehead tags", "fail-closed provenance fields"],
        },
        "comparisonEvidence": {
            "sourceScanInspected": True,
            "renderedSourcePath": "work/omr/452b-martin/source.jpg",
            "renderedDraftInputs": ["work/omr/452b-martin/source.mxl"],
        ...
        "blockingReason": "Autonomous promotion is blocked by the source/OMR meter conflict, named duration/event failures, blank source-visible groups, absent OMR key metadata, and only the watermark-intersected middle-system events. The derivative preserves detected events but does not invent rhythm, pitch, or lyrics.",
        "nextAction": "autonomous-promotion-blocked-by-source-meter-and-event-grouping; requires-source-event-verification",
