#!/usr/bin/env python3
"""Reconcile Cunningham's retained scan OMR into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/561-cunningham/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/561-cunningham-0c0d2be29e.jpg"
RAW_OMR = ROOT / "work/omr/561-cunningham/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/561-cunningham-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/561-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, raw_hash: str) -> bytes:
    ...
    for part in children(root, "part"):
        ...
        # The scan prints E Minor: one sharp.
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/561",
        ...
        "atlas-repeat-ending-encoding": "not encoded; source-visible repeats are not independently reconstructed",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies CUNNINGHAM. 8s & 7s., E minor, 3/2, Thomas Hastings (1824), Robert Stoddard (2011), four vocal parts, and two source-visible lyric stanzas. Repeat bars are visible in the first section and terminal double bars close the second; a diagonal DO NOT COPY watermark intersects the middle systems.",
        "Audiveris reports 13 raw measures across two systems (6 and 7), while the retained MXL exports 12 measures per part. The retained score contains 144 pitched events, 5 empty exported measures, and 45 of 48 exported part-measures fail the 3/2 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The OMR run also records an internal graph-processing failure and forced exit in its audit log. The retained OMR and corrected derivative contain no lyrics, repeat elements, or per-note four-shape encoding. The E-minor key and 3/2 meter are source observations only; adding those attributes does not prove the underlying OMR events or printed shapes. No unsupported event, lyric, repeat, or watermark-obscured material was fabricated.",
        "No authorized exact-edition structured Cunningham witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, endings, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/561", "edition": "Sacred Harp, 2025 Edition", "songNo": "561", "title": "Cunningham",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=561", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/561-Cunningham/561.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "CUNNINGHAM. 8s & 7s.", "composer": "Thomas Hastings, 1824", "arranger": "Robert Stoddard, 2011", "key": "E minor", "mode": "minor", "timeSignature": "3/2", "meter": "8s & 7s", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 13, "sourceRawMeasuresBySystem": [6, 7], "exportedMeasuresByPart": {"P1": 12, "P2": 12, "P3": 12, "P4": 12},
                "sourceLyricsVisible": True, "sourceStanzaCount": 2, "repeatBarsVisible": True, "numberedEndingsVisible": False, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True,
            },
        },
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Cunningham witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromRetainedOmr": True, "status": "review-only-not-source-verified", "corrections": ["source-observed E-minor key", "source-observed 3/2 meter", "explicit autonomous-block metadata", "lyrics/repeats/shapes intentionally not fabricated"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 13-versus-12 measure topology discrepancy, 45-of-48 retained-OMR 3/2 duration failures, 5 empty exported measures, OMR graph-processing failure, absent lyrics/repeat semantics/per-note shape encoding, watermark-intersected notation, differing source-image hashes, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
