...
SOURCE_IMAGE = ROOT / "work/omr/449-lovely-social-band/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/449-lovely-social-band-b2e12bf325.jpg"
...
def main() -> int:
    ...
    blocking = [
        ...
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. The 150 four-shape noteheads are derived from OMR pitch steps plus the observed F-major key, not independently verified against every printed notehead. No unsupported event, lyric, or repeat was fabricated.",
    ...
    audit = {
        ...
        "sourceAuthority": {
            ...
            "directObservations": {"header": "LOVELY SOCIAL BAND. L.M.D.", "composer": "John Poage Campbell, 1806", "arranger": "Richard Mayers, 2022", "key": "F major", "mode": "major", "timeSignature": "6/8", "meter": "Long Meter Double (8,8,8,8,8,8,8,8)", "parts": 4, "sourceRawMeasuresFromAudiveris": 16, "sourceRawMeasuresBySystem": [7, 9], "exportedMeasuresByPart": {"P1": 16, "P2": 16, "P3": 16, "P4": 16}, "sourceLyricsVisible": True, "numberedVersesVisible": True, "repeatBarsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True},
        },
        ...
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromPriorReviewDraft": True, "status": "review-only-not-source-verified", "corrections": ["source-observed F-major mode", "source-observed 6/8 meter", "explicit autonomous-block metadata", "preserved derived four-shape tags without claiming per-note verification", "lyrics and uncertain repeat semantics intentionally omitted"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 62-of-64 normalized-v2 6/8 duration failures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending encoding, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
