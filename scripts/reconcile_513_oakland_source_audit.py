#!/usr/bin/env python3
"""Reconcile Oakland's retained shape draft into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/513-oakland/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/513-oakland-1d38036631.jpg"
RAW_OMR = ROOT / "work/omr/513-oakland/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-513-oakland-1d38036631/work__source-images__2025__513-oakland-1d38036631.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/513-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/513-oakland-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/513-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/513",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies OAKLAND. C.M., E minor, 4/4, Isaac Watts (1707), David Smead (2017), four vocal parts, and source-visible lyrics. A repeat with numbered first/second endings is visible, the page ends with a terminal double bar, and a diagonal DO NOT COPY watermark intersects the middle and lower systems.",
        "Audiveris reports 14 raw measures (7 in the first system and 7 in the second), while the retained source MXL and normalized-v2 export 12 measures per part. The normalized-v2 working score contains 107 pitched events, 10 empty exported measures, and 42 of 48 exported part-measures fail the 4/4 duration target at divisions=1. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 107 four-shape noteheads are derived from OMR pitch steps plus the observed E-minor key, not independently verified against every printed notehead. The requested immutable scan and retained catalog image share geometry but have different hashes; neither was silently substituted for the other. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "No authorized exact-edition structured Oakland witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/513", "edition": "Sacred Harp, 2025 Edition", "songNo": "513", "title": "Oakland",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=513", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/513-Oakland/513.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "OAKLAND. C.M.", "composer": "Isaac Watts, 1707", "arranger": "David Smead, 2017", "key": "E minor", "mode": "minor", "timeSignature": "4/4", "meter": "Common Meter (8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 14, "sourceRawMeasuresBySystem": [7, 7], "exportedMeasuresByPart": {"P1": 12, "P2": 12, "P3": 12, "P4": 12},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Oakland witness was available; alternate editions were not used."},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 14-versus-12 measure topology discrepancy, 42-of-48 normalized-v2 4/4 duration failures, 10 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-intersected notation, differing source-image hashes, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
