#!/usr/bin/env python3
"""Create a source-derived, fail-closed Revere derivative."""
...
SOURCE_IMAGE = ROOT / "work/omr/242-revere/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/242-revere-dc6ac204cb.jpg"
SOURCE = ROOT / "work/omr/242-revere/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/242-revere-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/242-revere-source-correction-v2-comparison.json"

# E minor uses the relative G-major four-shape spelling.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    fields = {
        "atlas-queue-id": "sh2025/242", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false",
        ...
        "atlas-source-key": "E minor", "atlas-source-mode": "minor", "atlas-source-time-signature": "4/4", "atlas-source-meter": "Common Meter Double (8,6,8,6,8,6,8,6)",
        "atlas-source-repeat-ending": "The source visibly has sectional repeats with numbered first/second endings and a terminal double bar; the retained OMR does not encode complete repeat/ending semantics, so none was fabricated.",
        "atlas-shape-encoding": "derived four-shape spelling from retained OMR pitch steps and source-visible E-minor key; not source-verified per event", "atlas-lyrics": "omitted; source lyrics are visible but retained OMR has no directly aligned lyric underlay",
        "atlas-provenance-policy": "immutable 2025 scan is authoritative; the retained source-image duplicate is same geometry but a distinct JPEG byte stream; no alternate witness was used; this OMR derivative is evidence only",
        "atlas-blocker": "The immutable page visibly prints REVERE. C.M.D., E minor, 4/4, Isaac Watts 1724, Micah John Walter 2016, four vocal parts, lyrics, sectional first/second endings, and a terminal double bar. The retained source OMR exports 26 measures per part but Audiveris reports 29 raw measures across three systems; its duration ends fail the 4/4 target in 75 of 104 measures, with no lyrics, no source-confirmed shapes, and no complete repeat/ending semantics. No unsupported notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly establishes Revere C.M.D., E minor, 4/4, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar.",
        "The retained source OMR exports 26 measures per part and 260 note events (258 pitched and 2 rests), but Audiveris reports 29 raw measures; 75 of 104 exported measures fail the 4/4 duration audit, so event timing and topology are not proven source-faithful.",
        "The retained source OMR has no lyrics, no four-shape notehead tags, and no complete source repeat/ending semantics. The derivative adds observed E-minor/4/4 metadata and derived shapes without rewriting uncertain events.",
        ...
        "The requested source.jpg and retained catalog image have identical geometry but different JPEG bytes; both hashes are preserved and the requested source.jpg remains authoritative.",
    ]
    audit = {
        "queueId": "sh2025/242", "edition": "Sacred Harp, 2025 Edition", "songNo": "242", "title": "Revere", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=242", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/200-299/242-Revere/242.jpg", "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": image_hash, "immutable": True, "directObservations": {"header": "REVERE. C.M.D.", "composer": "Isaac Watts, 1724", "arranger": "Micah John Walter, 2016", "key": "E minor", "mode": "minor", "timeSignature": "4/4", "meter": "Common Meter Double (8,6,8,6,8,6,8,6)", "parts": 4, "measuresByPart": {"P1": 26, "P2": 26, "P3": 26, "P4": 26}, "lyricsVisible": True, "repeatBarsVisible": True, "endingsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True}, "retainedSourceImageDuplicate": {"path": str(RETAINED_IMAGE.relative_to(ROOT)), "sha256": retained_hash, "immutable": True, "geometryMatchesRequestedSource": True, "byteEqualToRequestedSource": False}},
        ...
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": source_events == corrected_events, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "sourceBarlines": source_barlines, "correctedBarlines": corrected_barlines, "corrections": ["source E-minor key and explicit minor mode", "source 4/4 time signature", "four-shape noteheads derived for every retained pitched event", "source lyric/repeat/watermark visibility recorded in provenance", "lyrics and uncertain repeat/ending semantics intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": image_hash, "retainedDuplicatePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedDuplicateSha256": retained_hash, "method": "full-resolution visual inspection of requested immutable source scan and retained duplicate plus retained source MXL and Audiveris audit; no alternate witness used to fill source events", "blockingFindings": blocking},
        "blockingFindings": blocking, "blockingReason": "Autonomous promotion is blocked by the 26-versus-29 measure topology discrepancy, duration failures in all exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-obscured source regions, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
