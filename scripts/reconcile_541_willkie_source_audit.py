#!/usr/bin/env python3
"""Reconcile Willkie's retained scan OMR into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/541-willkie/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/541-willkie-35a9bceadd.jpg"
RAW_OMR = ROOT / "work/omr/541-willkie/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/541-willkie-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/541-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, raw_hash: str) -> bytes:
    ...
    for part in children(root, "part"):
        ...
        # The scan prints D Minor: one flat.
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies WILLKIE. C.M.D., D minor, 4/4, Isaac Watts (1709), T.D. George (2018), four vocal parts, and source-visible lyrics. Repeat bars with numbered first/second endings and a terminal double bar are visible; a diagonal DO NOT COPY watermark intersects the middle/lower systems.",
        "Audiveris reports 23 raw measures across two systems (12 and 11), while the retained MXL exports 21 measures per part. The retained score contains 193 pitched events, 16 empty exported measures, and 69 of 84 exported part-measures fail the 4/4 duration target at divisions=1. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics, repeat/ending elements, or per-note four-shape encoding. The D-minor key and 4/4 meter are source observations only; adding those attributes does not prove the underlying OMR events or printed shapes. The current source image and retained catalog image have different hashes, so neither is silently substituted for the other. No unsupported event, lyric, repeat, ending, or watermark-obscured material was fabricated.",
        "No authorized exact-edition structured Willkie witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, endings, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/541", "edition": "Sacred Harp, 2025 Edition", "songNo": "541", "title": "Willkie",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=541", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/541-Willkie/541.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "WILLKIE. C.M.D.", "composer": "Isaac Watts, 1709", "arranger": "T.D. George, 2018", "key": "D minor", "mode": "minor", "timeSignature": "4/4", "meter": "Common Meter Double (8,6,8,6,8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 23, "sourceRawMeasuresBySystem": [12, 11], "exportedMeasuresByPart": {"P1": 21, "P2": 21, "P3": 21, "P4": 21},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Willkie witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromRetainedOmr": True, "status": "review-only-not-source-verified", "corrections": ["source-observed D-minor key", "source-observed 4/4 meter", "explicit autonomous-block metadata", "lyrics/repeats/endings/shapes intentionally not fabricated"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 23-versus-21 measure topology discrepancy, 69-of-84 retained-OMR 4/4 duration failures, 16 empty exported measures, absent lyrics/repeat/ending semantics/per-note shape encoding, watermark-intersected notation, differing source-image hashes, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
