#!/usr/bin/env python3
"""Create a source-derived, fail-closed Stafford derivative."""
...
SOURCE = ROOT / "work/omr/78-stafford/source.mxl"
SOURCE_IMAGE = ROOT / "work/omr/78-stafford/source.jpg"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/78-stafford-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/78-stafford-source-correction-v2-comparison.json"

# A major uses the A-major diatonic four-shape spelling: A=fa, B=sol,
# C-sharp=la, D=fa, E=sol, F-sharp=la, G-sharp=mi. Tags are derived from
# retained OMR pitch steps and are not evidence that the OMR pitches are right.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    fields = {
        ...
        "atlas-source-repeat-ending": "Printed repeat bars and 1/2 endings are visible on the immutable page; retained OMR does not encode a complete source repeat/ending witness",
        ...
        "atlas-blocker": "The source page visibly prints STAFFORD. S.M., A Major, Isaac Watts 1719, four vocal parts, lyrics, and repeat/ending notation. Retained OMR exports 19 measures per part while Audiveris logged 20 raw measures; it lacks usable divisions, complete duration proof, lyrics, shapes, and repeat/ending structure. Its encoded two-sharp key conflicts with the source-visible A-major header. No notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly prints STAFFORD. S.M., A Major, Isaac Watts 1719, four vocal parts, lyric underlay, and repeat/ending notation.",
        "The retained source OMR exports 19 measure elements per part with event counts P1=34, P2=24, P3=35, P4=29, including three rests and nine empty measures; Audiveris reports 20 raw measures, so complete source topology is not established.",
        ...
        "The retained OMR has no lyrics, four-shape tags, or complete repeat/ending semantics. Its two-sharp encoded key conflicts with the source-visible A-major header; the derivative records the scan and does not trust the stale OMR key.",
        "No authorized same-title structured candidate exists, so no alternate tune or edition was used to fill missing notation.",
        "No separate retained work/source-images/2025 duplicate matching 78 Stafford exists; the immutable work/omr/78-stafford/source.jpg is the canonical local visual witness.",
    ]
    audit = {
        "queueId": "sh2025/78",
        ...
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=78",
            ...
            "directObservations": {
                ...
                "endingsVisible": True,
                "sourceMeasureCountStatus": "Audiveris direct page analysis reports 20 raw measures (10 per system); exported OMR contains 19 per part and does not establish complete source topology",
            },
            "retainedSourceImageMissing": {"expectedGlob": "work/source-images/2025/*78*stafford*.jpg", "status": "not-found"},
        },
        "inputOmr": {"path": "work/omr/78-stafford/source.mxl", "sha256": source_hash, "status": "retained-source-scan-omr"},
        "candidateWitness": {"status": "none-authorized", "sameTitleStructuredCandidate": False},
        "correctedDraft": {
            "path": "work/omr/autonomous-transcriptions/2025/78-stafford-source-correction-v2.mxl",
            "sha256": draft_hash,
            "summary": summary,
    ...
    print(json.dumps({"record": "sh2025/78", "status": audit["comparisonStatus"], "sourceImageSha256": image_hash, "inputOmrSha256": source_hash, "correctedDraftSha256": draft_hash, "summary": summary, "audit": str(AUDIT.relative_to(ROOT))}, ensure_ascii=False, sort_keys=True))
    return 0
