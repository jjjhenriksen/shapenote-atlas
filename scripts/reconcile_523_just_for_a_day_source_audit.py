#!/usr/bin/env python3
"""Reconcile Just for a Day's retained scan OMR into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/523-just-for-a-day/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/523-just-for-a-day-3bf39235e1.jpg"
RAW_OMR = ROOT / "work/omr/523-just-for-a-day/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/523-just-for-a-day-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/523-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, raw_hash: str) -> bytes:
    ...
    for part in children(root, "part"):
        ...
        # The scan prints D Major: two sharps.
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/523",
        ...
        "atlas-lyrics-encoding": "not encoded; three source-visible stanzas are not directly syllable-aligned",
        "atlas-repeat-ending-encoding": "not encoded; source-visible terminal ending is not independently reconstructed",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies JUST FOR A DAY. L.M., D major, 2/4, A. R. Walton (1909), four vocal parts, and three source-visible numbered stanzas. A terminal double bar is visible and a diagonal DO NOT COPY watermark intersects the middle system.",
        "Audiveris reports 17 raw measures in one system, while the retained MXL exports 16 measures per part. The retained score contains 69 pitched events, 25 empty exported measures, and 39 of 64 exported part-measures fail the 2/4 duration target at divisions=1. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics, barline/repeat semantics, or per-note four-shape encoding. The D-major key and 2/4 meter are source observations only; adding those attributes does not prove the underlying OMR events or printed shapes. No unsupported event, lyric, ending, or watermark-obscured material was fabricated.",
        "No authorized exact-edition structured Just for a Day witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, endings, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/523", "edition": "Sacred Harp, 2025 Edition", "songNo": "523", "title": "Just for a Day",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=523", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/523-Just-for-a-Day/523.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "JUST FOR A DAY. L.M.", "composer": "A. R. Walton, 1909", "arranger": "A. R. Walton, 1909", "key": "D major", "mode": "major", "timeSignature": "2/4", "meter": "Long Meter (8,8,8,8)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 17, "sourceRawMeasuresBySystem": [17], "exportedMeasuresByPart": {"P1": 16, "P2": 16, "P3": 16, "P4": 16},
                "sourceLyricsVisible": True, "sourceStanzaCount": 3, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True,
            },
        },
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Just for a Day witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromRetainedOmr": True, "status": "review-only-not-source-verified", "corrections": ["source-observed D-major key", "source-observed 2/4 meter", "explicit autonomous-block metadata", "lyrics/endings/shapes intentionally not fabricated"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 17-versus-16 measure topology discrepancy, 39-of-64 retained-OMR 2/4 duration failures, 25 empty exported measures, absent lyrics/barline semantics/per-note shape encoding, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
        ...
        "policy": "Immutable 2025 source images remain authoritative. OMR and metadata corrections cannot authorize promotion without direct event-level, rhythm, lyric, ending, mode, meter, and shape proof.",
