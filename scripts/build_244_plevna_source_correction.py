#!/usr/bin/env python3
"""Create a source-derived, fail-closed Plevna derivative."""
...
SOURCE_IMAGE = ROOT / "work/omr/244-plevna/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/244-plevna-faef20b4bf.jpg"
SOURCE = ROOT / "work/omr/244-plevna/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/244-plevna-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/244-plevna-source-correction-v2-comparison.json"

# G major four-shape spelling: G=fa, A=sol, B=la, C=fa, D=sol, E=la, F-sharp=mi.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    fields = {
        "atlas-queue-id": "sh2025/244", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false",
        ...
        "atlas-source-key": "G major", "atlas-source-mode": "major", "atlas-source-time-signature": "4/4", "atlas-source-meter": "Common Meter Double (8,6,8,6,8,6,8,6)",
        ...
        "atlas-shape-encoding": "derived four-shape spelling from retained OMR pitch steps and source-visible G-major key; not source-verified per event", "atlas-lyrics": "omitted; source lyrics are visible but retained OMR has no directly aligned lyric underlay",
        ...
        "atlas-blocker": "The immutable page visibly prints PLEVNA. C.M.D., G major, 4/4, Isaac Watts 1715, Brad Bahler 2013, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar. The retained source OMR exports 18 measures per part but Audiveris reports 20 raw measures across three systems; its duration ends fail the 4/4 target in 58 of 72 measures, with no lyrics, no source-confirmed shapes, and no complete repeat/ending semantics. The watermark intersects lower systems. No unsupported notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly establishes Plevna C.M.D., G major, 4/4, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar.",
        "The retained source OMR exports 18 measures per part and 238 note events (232 pitched and 6 rests), but Audiveris reports 20 raw measures; 58 of 72 exported measures fail the 4/4 duration audit, so event timing and topology are not proven source-faithful.",
        "The retained source OMR has no lyrics, no four-shape notehead tags, and no complete source repeat/ending semantics. The derivative adds observed G-major/4/4 metadata and derived shapes without rewriting uncertain events.",
        ...
        "The requested source.jpg and retained catalog image are preserved with separate hashes; the retained duplicate is not substituted for the requested source authority.",
    ]
    audit = {
        "queueId": "sh2025/244", "edition": "Sacred Harp, 2025 Edition", "songNo": "244", "title": "Plevna", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=244", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/200-299/244-Plevna/244.jpg", "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": image_hash, "immutable": True, "directObservations": {"header": "PLEVNA. C.M.D.", "composer": "Isaac Watts, 1715", "arranger": "Brad Bahler, 2013", "key": "G major", "mode": "major", "timeSignature": "4/4", "meter": "Common Meter Double (8,6,8,6,8,6,8,6)", "parts": 4, "measuresByPart": {"P1": 18, "P2": 18, "P3": 18, "P4": 18}, "lyricsVisible": True, "repeatBarsVisible": True, "endingsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True}, "retainedSourceImageDuplicate": {"path": str(RETAINED_IMAGE.relative_to(ROOT)), "sha256": retained_hash, "immutable": True, "geometryMatchesRequestedSource": True, "byteEqualToRequestedSource": False}},
        ...
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": source_events == corrected_events, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "sourceBarlines": source_barlines, "correctedBarlines": corrected_barlines, "corrections": ["source G-major key and explicit major mode", "source 4/4 time signature", "four-shape noteheads derived for every retained pitched event", "source lyric/repeat/watermark visibility recorded in provenance", "lyrics and uncertain repeat/ending semantics intentionally omitted"]},
        ...
        "blockingFindings": blocking, "blockingReason": "Autonomous promotion is blocked by the 18-versus-20 measure topology discrepancy, 58 duration failures in the exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-obscured lower systems, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
