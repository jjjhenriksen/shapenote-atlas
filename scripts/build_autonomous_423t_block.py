#!/usr/bin/env python3
"""Create a fail-closed, source-derived Grantville OMR derivative."""
...
SOURCE = ROOT / "work" / "omr" / "423t-grantville" / "source.mxl"
IMAGE = ROOT / "work" / "omr" / "423t-grantville" / "source.jpg"
OUTPUT = ROOT / "work" / "omr" / "autonomous-transcriptions" / "2025" / "423t-autonomous-blocked.mxl"
AUDIT = ROOT / "work" / "source-transcriptions" / "2025" / "423t-grantville-autonomous-comparison.json"
...
def transform() -> tuple[bytes, dict[str, object]]:
    with zipfile.ZipFile(SOURCE) as archive:
        ...
        for name, value in {
            "atlas-queue-id": "sh2025/423t",
            "atlas-transcription-status": "autonomously-blocked",
            "atlas-safe-to-promote": "false",
            "atlas-source-image": "work/omr/423t-grantville/source.jpg",
            ...
            "atlas-blocker": "The source visibly has four parts and ten 4/4 measures, but retained OMR duration/event grouping fails at P1 m1=2,m2=2,m3=6,m4=2,m6=0,m7=20; P2 m1=2,m4=6,m5=6,m7=24,m10=3; P3 m3=3,m4=5,m5=5,m6=1,m7=32,m8=1,m9=2,m10=13; and P4 m1=2,m2=2,m4=2,m6=7,m7=30,m8=2,m10=3. P1 m6 is blank while the source shows notation. OMR key metadata is inconsistent with the source-visible F-sharp minor (P1 records fifths=2 and P3 fifths=1, with no source mode). Watermark intersections are separately unresolved only in the middle systems.",
        }.items():
...
def main() -> int:
    ...
    blocking = [
        "The retained source scan visibly prints GRANTVILLE. C.M., F-sharp minor, 4/4, four vocal parts, and ten measures per part.",
        "The retained OMR has ten measures per part, but its duration totals fail in the named part/measure groups recorded in the derivative; P1 m6 is empty although the source visibly contains notation there.",
        "The retained OMR includes a collapsed high-density m7 region across all four parts; the derivative preserves those detected events without inventing their source rhythm or missing events.",
        "The retained OMR key metadata conflicts with the source-visible F-sharp minor: P1 records fifths=2 and P3 records fifths=1, with no source mode; the derivative corrects the displayed source metadata without silently rewriting event pitches.",
    ...
    audit = {
        ...
        "inputOmr": {
            "path": "work/omr/423t-grantville/source.mxl",
            "sha256": source_hash,
            "status": "retained-source-scan-omr",
        },
        "correctedDraft": {
            "path": "work/omr/autonomous-transcriptions/2025/423t-autonomous-blocked.mxl",
            "sha256": output_hash,
            "summary": summary,
            "corrections": [
        ...
        "comparisonEvidence": {
            ...
            "renderedSourcePath": "work/omr/423t-grantville/source.jpg",
            "renderedDraftInputs": ["work/omr/423t-grantville/source.mxl"],
            "method": "direct visual inspection of retained scan plus structural/event and duration audit of retained OMR",
            "blockingFindings": blocking,
        },
        "blockingReason": "Autonomous promotion is blocked by the named duration/event failures, the blank P1 m6, the collapsed m7 groups, the conflicting OMR key metadata, and only the watermark-intersected middle-system events. The derivative preserves detected events but does not invent rhythm, pitch, or lyrics.",
