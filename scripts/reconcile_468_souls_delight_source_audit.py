#!/usr/bin/env python3
"""Reconcile Soul's Delight's retained shape draft into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/468-soul-s-delight/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/468-soul-s-delight-05b51d5cb4.jpg"
RAW_OMR = ROOT / "work/omr/468-soul-s-delight/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-468-soul-s-delight-05b51d5cb4/work__source-images__2025__468-soul-s-delight-05b51d5cb4.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/468-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/468-soul-s-delight-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/468-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    for part in children(root, "part"):
        ...
        # The scan prints F Major; F is one flat.
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/468",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies SOUL’S DELIGHT. C.M.D., F major, 4/4, T.D. George (2015), four vocal parts, source-visible lyrics, repeat bars with numbered first/second endings, and a terminal double bar; a diagonal DO NOT COPY watermark intersects the middle systems.",
        "Audiveris reports 21 raw measures (7 in the first system, 6 in the second, and 8 in the third), while the retained source MXL and normalized-v2 export 20 measures per part. The normalized-v2 working score contains 223 pitched events, 10 empty exported measures, and 72 of 80 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 223 four-shape noteheads are derived from OMR pitch steps plus the observed F-major key, not independently verified against every printed notehead. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "No authorized exact-edition structured Soul’s Delight witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/468", "edition": "Sacred Harp, 2025 Edition", "songNo": "468", "title": "Soul’s Delight",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=468", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/468-Souls-Delight/468.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "SOUL’S DELIGHT. C.M.D.", "composer": "T.D. George, 2015", "arranger": "T.D. George, 2015", "key": "F major", "mode": "major", "timeSignature": "4/4", "meter": "Common Meter Double (8,6,8,6,8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 21, "sourceRawMeasuresBySystem": [7, 6, 8], "exportedMeasuresByPart": {"P1": 20, "P2": 20, "P3": 20, "P4": 20},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Soul’s Delight witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromPriorReviewDraft": True, "status": "review-only-not-source-verified", "corrections": ["source-observed F-major mode", "source-observed 4/4 meter", "explicit autonomous-block metadata", "preserved derived four-shape tags without claiming per-note verification", "lyrics and repeat/ending semantics intentionally omitted"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 21-versus-20 measure topology discrepancy, 72-of-80 normalized-v2 4/4 duration failures, 10 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
