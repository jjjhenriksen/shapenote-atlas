#!/usr/bin/env python3
"""Create a source-derived, fail-closed Red Sands derivative."""
...
SOURCE_IMAGE = ROOT / "work/omr/429-red-sands/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/429-red-sands-0eee48c100.jpg"
SOURCE = ROOT / "work/omr/429-red-sands/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/429-red-sands-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/429-red-sands-source-correction-v2-comparison.json"

# F-major four-shape spelling: F=fa, G=sol, A=la, Bb=fa, C=sol, D=la, E=mi.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    summary: dict[str, object] = {"parts": len(parts), "measuresByPart": {}, "eventsByPart": {}, "pitchedEvents": 0, "restEvents": 0, "shapeNoteheadsAdded": 0, "lyricsRetained": 0, "emptyMeasures": 0, "divisionsByPart": {}, "durationEndByPart": {}, "durationFailuresAgainst4_4": {}, "sourceBarlines": source_barlines}
    for part in parts:
        ...
        summary["divisionsByPart"][part_id] = divisions  # type: ignore[index]
        ...
        summary["durationFailuresAgainst4_4"][part_id] = [f"m{number}={end}" for number, end in ends.items() if end != target_duration]  # type: ignore[index]
    ...
    fields = {
        "atlas-queue-id": "sh2025/429", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false",
        ...
        "atlas-source-key": "F major", "atlas-source-mode": "major", "atlas-source-time-signature": "4/4", "atlas-source-meter": "Common Particular Meter (8,8,6,8,8,6)",
        ...
        "atlas-shape-encoding": "derived four-shape spelling from retained OMR pitch steps and source-visible F-major key; not source-verified per event", "atlas-lyrics": "omitted; source lyrics are visible but retained OMR has no directly aligned lyric underlay",
        ...
        "atlas-blocker": "The immutable page visibly prints RED SANDS. C.P.M., F major, 4/4, Charles Wesley 1747, Barry Parsons 2018, four vocal parts across 22 source measures, lyrics, numbered first/second endings, a terminal double bar, and a diagonal DO NOT COPY watermark crossing central notation and lyrics. The retained source OMR exports 21 measures per part, 146 pitched events, 28 empty measures, and duration ends failing the 4/4 target in 76 of 84 exported measures at divisions=3 resolution; its Audiveris log also records a key-export exception. It has no lyrics, no source-confirmed shapes, and no complete repeat/ending semantics. No unsupported notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly establishes Red Sands C.P.M., F major, 4/4, four vocal parts across 22 source measures, lyrics, numbered first/second endings, a terminal double bar, and a diagonal DO NOT COPY watermark crossing central notation and lyrics.",
        "The retained source OMR exports 21 measures per part and 146 pitched events, while Audiveris reports 22 raw measures; at divisions=3, 76 of 84 exported measures fail the 4/4 duration audit and 28 measures are empty, so event timing and topology are not proven source-faithful.",
        "The retained source OMR has no lyrics, no four-shape notehead tags, and no complete source repeat/ending semantics; its Audiveris log records a key-export exception. The derivative adds observed F-major/4/4 metadata and derived shapes without rewriting uncertain events.",
    ...
    audit = {
        "queueId": "sh2025/429", "edition": "Sacred Harp, 2025 Edition", "songNo": "429", "title": "Red Sands", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=429", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/429-Red-Sands/429.jpg", "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": image_hash, "immutable": True, "directObservations": {"header": "RED SANDS. C.P.M.", "composer": "Charles Wesley, 1747", "arranger": "Barry Parsons, 2018", "key": "F major", "mode": "major", "timeSignature": "4/4", "meter": "Common Particular Meter (8,8,6,8,8,6)", "parts": 4, "measuresByPart": {"P1": 22, "P2": 22, "P3": 22, "P4": 22}, "sourceRawMeasuresFromAudiveris": 22, "lyricsVisible": True, "repeatBarsVisible": True, "endingsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True}, "retainedSourceImageDuplicate": {"path": str(RETAINED_IMAGE.relative_to(ROOT)), "sha256": retained_hash, "immutable": True, "geometryMatchesRequestedSource": True, "byteEqualToRequestedSource": False}},
        ...
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": source_events == corrected_events, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "sourceBarlines": source_barlines, "correctedBarlines": corrected_barlines, "corrections": ["source F-major key and explicit major mode", "source 4/4 time signature", "four-shape noteheads derived for every retained pitched event", "source lyric/repeat/watermark visibility recorded in provenance", "lyrics and uncertain repeat/ending semantics intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": image_hash, "retainedDuplicatePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedDuplicateSha256": retained_hash, "method": "full-resolution visual inspection of requested immutable Red Sands scan and retained distinct image plus retained source MXL and Audiveris audit; no alternate witness used to fill source events", "blockingFindings": blocking},
        "blockingFindings": blocking, "blockingReason": "Autonomous promotion is blocked by the 21-versus-22 measure topology discrepancy, 76 duration failures in the exported measures at divisions=3, 28 empty measures, the Audiveris key-export exception, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-obscured central systems, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
