#!/usr/bin/env python3
"""Create a source-derived, fail-closed Rebekah derivative."""
...
SOURCE_IMAGE = ROOT / "work/omr/393-rebekah/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/393-rebekah-7b2c5d9f8b.jpg"
SOURCE = ROOT / "work/omr/393-rebekah/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/393-rebekah-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/393-rebekah-source-correction-v2-comparison.json"

# E major four-shape spelling: E=fa, F-sharp=sol, G-sharp=la, A=fa, B=sol, C-sharp=la, D-sharp=mi.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    for part in parts:
        ...
        summary["durationFailuresAgainst4_4"][part_id] = [f"m{number}={end}" for number, end in ends.items() if end != 4]  # type: ignore[index]
    ...
    fields = {
        "atlas-queue-id": "sh2025/393", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false",
        ...
        "atlas-source-key": "E major", "atlas-source-mode": "major", "atlas-source-time-signature": "4/4", "atlas-source-meter": "Common Meter (8,6,8,6)",
        ...
        "atlas-shape-encoding": "derived four-shape spelling from retained OMR pitch steps and source-visible E-major key; not source-verified per event", "atlas-lyrics": "omitted; source lyrics are visible but retained OMR has no directly aligned lyric underlay",
        ...
        "atlas-blocker": "The immutable page visibly prints REBEKAH. C.M., E major, 4/4, Rippon's Selection 1800, Timothy R. Gilmore 1990, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar. The retained source OMR exports 14 measures per part but Audiveris reports 16 raw measures across two systems; its duration ends fail the 4/4 target in 40 of 56 measures, with 15 empty measures, no lyrics, no source-confirmed shapes, and no complete repeat/ending semantics. The watermark intersects central notation. No unsupported notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly establishes Rebekah C.M., E major, 4/4, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar.",
        "The retained source OMR exports 14 measures per part and 133 note events (129 pitched and 4 rests), but Audiveris reports 16 raw measures; 40 of 56 exported measures fail the 4/4 duration audit and 15 measures are empty, so event timing and topology are not proven source-faithful.",
        "The retained source OMR has no lyrics, no four-shape notehead tags, and no complete source repeat/ending semantics. The derivative adds observed E-major/4/4 metadata and derived shapes without rewriting uncertain events.",
    ...
    audit = {
        "queueId": "sh2025/393", "edition": "Sacred Harp, 2025 Edition", "songNo": "393", "title": "Rebekah", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=393", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/300-399/393-Rebekah/393.jpg", "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": image_hash, "immutable": True, "directObservations": {"header": "REBEKAH. C.M.", "composer": "Rippon's Selection, 1800", "arranger": "Timothy R. Gilmore, 1990", "key": "E major", "mode": "major", "timeSignature": "4/4", "meter": "Common Meter (8,6,8,6)", "parts": 4, "measuresByPart": {"P1": 14, "P2": 14, "P3": 14, "P4": 14}, "lyricsVisible": True, "repeatBarsVisible": True, "endingsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True}, "retainedSourceImageDuplicate": {"path": str(RETAINED_IMAGE.relative_to(ROOT)), "sha256": retained_hash, "immutable": True, "geometryMatchesRequestedSource": True, "byteEqualToRequestedSource": True}},
        ...
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": source_events == corrected_events, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "sourceBarlines": source_barlines, "correctedBarlines": corrected_barlines, "corrections": ["source E-major key and explicit major mode", "source 4/4 time signature", "four-shape noteheads derived for every retained pitched event", "source lyric/repeat/watermark visibility recorded in provenance", "lyrics and uncertain repeat/ending semantics intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": image_hash, "retainedDuplicatePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedDuplicateSha256": retained_hash, "method": "full-resolution visual inspection of requested immutable Rebekah scan and retained duplicate plus retained source MXL and Audiveris audit; no alternate witness used to fill source events", "blockingFindings": blocking},
        "blockingFindings": blocking, "blockingReason": "Autonomous promotion is blocked by the 14-versus-16 measure topology discrepancy, 40 duration failures in the exported measures, 15 empty measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-obscured central systems, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
