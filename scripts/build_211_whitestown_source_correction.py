#!/usr/bin/env python3
"""Create a source-derived, fail-closed Whitestown derivative."""
...
SOURCE = ROOT / "work/omr/211-whitestown/source.mxl"
SOURCE_IMAGE = ROOT / "work/omr/211-whitestown/source.jpg"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/211-whitestown-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/211-whitestown-source-correction-v2-comparison.json"

# E minor uses the relative-major four-shape spelling: E=la, F#=mi,
# G=fa, A=sol, B=la, C=fa, D=sol. The tags describe the retained OMR
# pitches; they are not evidence that the OMR event stream is source-correct.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    for part in parts:
        ...
        summary["durationFailuresAgainst4_4"][part_id] = [  # type: ignore[index]
            f"m{number}={end}" for number, end in ends.items() if end != 24
        ]
    ...
    for key, value in {
        ...
        "atlas-source-meter": "Long Meter Double (L.M.D.)",
        "atlas-source-repeat-ending": "Source page visibly contains a repeat and numbered 1/2 endings; retained OMR has no barlines, repeats, or ending elements, so they were not fabricated.",
        ...
        "atlas-provenance-policy": "immutable 2025 scan is authoritative; catalog G-major metadata is contradicted by the page's E-minor header; this OMR derivative is evidence only",
        "atlas-blocker": "The immutable page visibly prints WHITESTOWN. L.M.D., E Minor, Isaac Watts 1719, Howd 1800, four vocal parts, lyrics, and numbered 1/2 endings. Audiveris reports 26 raw measures across two systems (15 and 11), while the retained export has 25 measures per part, sparse events, no barlines/endings, no lyrics, and no encoded mode. The catalog queue's G-major key is stale/conflicting; the source page is authoritative. No notation was synthesized.",
    }.items():
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly prints WHITESTOWN. L.M.D., E minor, Isaac Watts 1719, Howd 1800, four vocal parts, lyrics, and numbered 1/2 endings.",
        "Audiveris reports 26 raw measures across two systems (15 in system 1 and 11 in system 2), but the retained source OMR exports only 25 measures per part; complete source topology is not established.",
        "The retained OMR has no barlines, repeats, or ending elements, no lyrics, no shape tags, and sparse event coverage. Its duration units are present but the missing barline topology prevents a complete source alignment audit; per-measure duration ends are retained in the audit.",
        "The retained OMR encodes one sharp but omits mode and time; the source page establishes E minor and 4/4. The catalog queue's G-major metadata is explicitly treated as stale/conflicting rather than used as notation authority.",
    ...
    audit = {
        ...
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=211",
            ...
            "immutable": True,
            "directObservations": {
                "header": "WHITESTOWN. L.M.D.",
                "composer": "Isaac Watts, 1719",
                "arranger": "Howd, 1800",
                "key": "E minor",
                "mode": "minor",
                "timeSignature": "4/4",
                "meter": "Long Meter Double (L.M.D.)",
                "parts": 4,
                "systems": 2,
                "lyricsVisible": True,
                ...
                "sourceMeasureCountStatus": "Audiveris direct page analysis reports 26 raw measures, 15 in system 1 and 11 in system 2; exported OMR contains 25 per part",
            },
            "retainedSourceImageDuplicate": {"expectedGlob": "work/source-images/2025/*211*whitestown*.jpg", "status": "not-found"},
        },
        "inputOmr": {"path": "work/omr/211-whitestown/source.mxl", "sha256": source_hash, "status": "retained-source-scan-omr"},
        "candidateWitness": {"status": "none-authorized", "sameTitleStructuredCandidate": False},
        "catalogMetadataConflict": {"catalogKey": "G major", "sourcePageKey": "E minor", "resolution": "source-page-authoritative"},
        "correctedDraft": {
            ...
            "corrections": [
                "source E-minor key and explicit minor mode",
                "source 4/4 time signature",
                "four-shape noteheads derived for every retained pitched event",
                "source lyrics/repeat/numbered-ending visibility recorded in provenance",
        ...
        "blockingReason": "Autonomous promotion is blocked by the 25-versus-26 measure discrepancy, incomplete event/topology evidence, absent lyric and repeat/ending encoding, missing source mode in the input, and conflicting catalog key metadata. The derivative preserves detected events and adds source metadata/derived shapes without fabrication.",
        "nextAction": "autonomous-promotion-blocked-by-incomplete-source-event-witness-and-conflicting-catalog-key; retain-corrected-draft-only",
    ...
    print(json.dumps({"record": "sh2025/211", "status": audit["comparisonStatus"], "sourceImageSha256": image_hash, "inputOmrSha256": source_hash, "correctedDraftSha256": draft_hash, "summary": summary, "audit": str(AUDIT.relative_to(ROOT))}, ensure_ascii=False, sort_keys=True))
    return 0
