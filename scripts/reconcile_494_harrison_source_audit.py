#!/usr/bin/env python3
"""Reconcile Harrison's retained shape draft into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/494-harrison/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/494-harrison-87df87c2b0.jpg"
RAW_OMR = ROOT / "work/omr/494-harrison/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-494-harrison-87df87c2b0/work__source-images__2025__494-harrison-87df87c2b0.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/494-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/494-harrison-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/494-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/494",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies HARRISON. S.M., C major, 4/4, Anne Steele (1760), T.D. George (2022), four vocal parts, and source-visible lyrics. A repeat with numbered first/second endings is visible, the page ends with a terminal double bar, and a diagonal DO NOT COPY watermark intersects the middle systems.",
        "Audiveris reports 15 raw measures (8 in the first system and 7 in the second), while the retained source MXL and normalized-v2 export 13 measures per part. The normalized-v2 working score contains 178 pitched events, 7 empty exported measures, and 44 of 52 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 178 four-shape noteheads are derived from OMR pitch steps plus the observed C-major key, not independently verified against every printed notehead. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "No authorized exact-edition structured Harrison witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/494", "edition": "Sacred Harp, 2025 Edition", "songNo": "494", "title": "Harrison",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=494", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/494-Harrison/494.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "HARRISON. S.M.", "composer": "Anne Steele, 1760", "arranger": "T.D. George, 2022", "key": "C major", "mode": "major", "timeSignature": "4/4", "meter": "Short Meter (6,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 15, "sourceRawMeasuresBySystem": [8, 7], "exportedMeasuresByPart": {"P1": 13, "P2": 13, "P3": 13, "P4": 13},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Harrison witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromPriorReviewDraft": True, "status": "review-only-not-source-verified", "corrections": ["source-observed C-major mode", "source-observed 4/4 meter", "explicit autonomous-block metadata", "preserved derived four-shape tags without claiming per-note verification", "lyrics and repeat/ending semantics intentionally omitted"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 15-versus-13 measure topology discrepancy, 44-of-52 normalized-v2 4/4 duration failures, 7 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
