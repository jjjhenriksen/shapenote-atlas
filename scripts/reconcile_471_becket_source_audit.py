#!/usr/bin/env python3
"""Reconcile Becket's retained shape draft into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/471-becket/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/471-becket-1a7c0f1d99.jpg"
RAW_OMR = ROOT / "work/omr/471-becket/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-471-becket-1a7c0f1d99/work__source-images__2025__471-becket-1a7c0f1d99.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/471-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/471-becket-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/471-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/471",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies BECKET. C.M., A major, 4/4, v.1 Simon Browne (1720), v.2 John Cennick (1764), Leah Velleman (2018), four vocal parts, two source-visible verses, repeat bars with numbered first/second endings, and a terminal double bar; a diagonal DO NOT COPY watermark intersects the middle systems.",
        "Audiveris reports 18 raw measures (10 in the first system and 8 in the second), while the retained source MXL exports 16 measures per part and normalized-v2 exports 17 measures per part. The normalized-v2 working score contains 178 pitched events, 16 empty exported measures, and 53 of 68 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 178 four-shape noteheads are derived from OMR pitch steps plus the observed A-major key, not independently verified against every printed notehead. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "No authorized exact-edition structured Becket witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/471", "edition": "Sacred Harp, 2025 Edition", "songNo": "471", "title": "Becket",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=471", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/471-Becket/471.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "BECKET. C.M.", "composer": "Simon Browne, 1720; John Cennick, 1764", "arranger": "Leah Velleman, 2018", "key": "A major", "mode": "major", "timeSignature": "4/4", "meter": "Common Meter (8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 18, "sourceRawMeasuresBySystem": [10, 8], "exportedMeasuresByPart": {"P1": 17, "P2": 17, "P3": 17, "P4": 17},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Becket witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromPriorReviewDraft": True, "status": "review-only-not-source-verified", "corrections": ["source-observed A-major mode", "source-observed 4/4 meter", "explicit autonomous-block metadata", "preserved derived four-shape tags without claiming per-note verification", "lyrics and repeat/ending semantics intentionally omitted"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 18-versus-17 normalized-v2 measure topology discrepancy (and 16-measure raw export), 53-of-68 normalized-v2 4/4 duration failures, 16 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
