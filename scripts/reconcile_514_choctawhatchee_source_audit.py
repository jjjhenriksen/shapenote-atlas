#!/usr/bin/env python3
"""Reconcile Choctawhatchee's retained scan OMR into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/514-choctawhatchee/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/514-choctawhatchee-3a0c514deb.jpg"
RAW_OMR = ROOT / "work/omr/514-choctawhatchee/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/514-choctawhatchee-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/514-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, raw_hash: str) -> bytes:
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/514",
        ...
        "atlas-lyrics-encoding": "not encoded; direct syllable alignment is not established",
        "atlas-repeat-ending-encoding": "not encoded; source-visible first/second endings are not independently reconstructed",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies CHOCTAWHATCHEE. 7s & 6s D., F major, 4/4, John B. Matthias (1836), Stanley Smith (1982), four vocal parts, and source-visible lyrics. A repeat with numbered first/second endings and a terminal double bar are visible; a diagonal watermark intersects the middle/lower systems.",
        "Audiveris reports 25 raw measures across three systems (10, 6, and 9), while the retained MXL exports 22 measures per part. The retained score contains 234 pitched events, 20 empty exported measures, and 77 of 88 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics, repeat/ending elements, or per-note four-shape encoding. The F-major key and 4/4 meter are source observations only; adding those attributes does not prove the underlying OMR events or printed shapes. No unsupported event, lyric, repeat, or watermark-obscured material was fabricated.",
        "No authorized exact-edition structured Choctawhatchee witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/514", "edition": "Sacred Harp, 2025 Edition", "songNo": "514", "title": "Choctawhatchee",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=514", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/514-Choctawhatchee/514.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "CHOCTAWHATCHEE. 7s & 6s D.", "composer": "John B. Matthias, 1836", "arranger": "Stanley Smith, 1982", "key": "F major", "mode": "major", "timeSignature": "4/4", "meter": "7s & 6s D.", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 25, "sourceRawMeasuresBySystem": [10, 6, 9], "exportedMeasuresByPart": {"P1": 22, "P2": 22, "P3": 22, "P4": 22},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Choctawhatchee witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromRetainedOmr": True, "status": "review-only-not-source-verified", "corrections": ["source-observed F-major key", "source-observed 4/4 meter", "explicit autonomous-block metadata", "lyrics/repeats/shapes intentionally not fabricated"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 25-versus-22 measure topology discrepancy, 77-of-88 retained-OMR 4/4 duration failures, 20 empty exported measures, absent lyrics/repeat semantics/per-note shape encoding, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
