#!/usr/bin/env python3
"""Reconcile Newbury's retained shape draft into an autonomous block.

The scan remains authoritative.  This script only carries the observed source
key and meter into a provenance-pinned derivative and records why the OMR
cannot enter the playable corpus yet.
"""
...
SOURCE_IMAGE = ROOT / "work/omr/453-newbury/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/453-newbury-69af7b15d8.jpg"
RAW_OMR = ROOT / "work/omr/453-newbury/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-453-newbury-69af7b15d8/work__source-images__2025__453-newbury-69af7b15d8.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/453-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/453-newbury-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/453-source-shape-autonomous-blocked-comparison.json"
...
def retime_4_4(summary: dict[str, object]) -> dict[str, object]:
    """Use the shared duration audit helper with an accurate 4/4 field name."""
    summary = retime_summary(summary, beats=4)
    failures = summary.pop("durationFailuresAgainst3_4", {})
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    for part in children(root, "part"):
        ...
        # The scan prints A Minor; A is zero fifths.
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/453",
...
def main() -> int:
    ...
    raw = retime_4_4(score_stats(RAW_OMR))
    working = retime_4_4(score_stats(WORKING_OMR))
    draft = retime_4_4(score_stats(OUTPUT))
    blocking = [
        "The immutable page visibly identifies NEWBURY. C.M., A minor, 4/4, The Divine Companion (1707), Samuel Turner (2022), four vocal parts, source-visible lyrics, repeat bars with first/second endings, and a terminal double bar; a diagonal DO NOT COPY watermark intersects the middle systems.",
        "Audiveris reports 18 raw measures (10 in the first system and 8 in the second), while the retained source MXL and normalized-v2 export 16 measures per part. The normalized-v2 working score contains 122 pitched events, 16 empty exported measures, and 52 of 64 exported part-measures fail the 4/4 duration target at divisions=1. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 122 four-shape noteheads are derived from OMR pitch steps plus the observed A-minor key, not independently verified against every printed notehead. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "No authorized exact-edition structured Newbury witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/453", "edition": "Sacred Harp, 2025 Edition", "songNo": "453", "title": "Newbury",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=453", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/453-Newbury/453.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "NEWBURY. C.M.", "composer": "The Divine Companion, 1707", "arranger": "Samuel Turner, 2022", "key": "A minor", "mode": "minor", "timeSignature": "4/4", "meter": "Common Meter (8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 18, "sourceRawMeasuresBySystem": [10, 8], "exportedMeasuresByPart": {"P1": 16, "P2": 16, "P3": 16, "P4": 16},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Newbury witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromPriorReviewDraft": True, "status": "review-only-not-source-verified", "corrections": ["source-observed A-minor mode", "source-observed 4/4 meter", "explicit autonomous-block metadata", "preserved derived four-shape tags without claiming per-note verification", "lyrics and repeat/ending semantics intentionally omitted"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 18-versus-16 measure topology discrepancy, 52-of-64 normalized-v2 4/4 duration failures, 16 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
