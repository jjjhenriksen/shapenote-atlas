#!/usr/bin/env python3
"""Create a source-derived, fail-closed Bethel derivative.

The immutable scan is authoritative.  This pass deliberately preserves the
retained OMR event stream instead of inventing the missing source notation.
"""
...
SOURCE = ROOT / "work/omr/27t-bethel/source.mxl"
SOURCE_IMAGE = ROOT / "work/omr/27t-bethel/source.jpg"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/27t-bethel-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/27t-bethel-source-correction-v2-comparison.json"

# F minor's relative major is A-flat major.  Four-shape syllables therefore
# follow the A-flat diatonic spelling: Ab=fa, Bb=sol, C=la, Db=fa, Eb=sol,
# F=la, G=mi.  These are derived tags, not proof that the OMR pitch is right.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    for part in parts:
        ...
        summary["durationFailuresAgainst4_4"][part_id] = [  # type: ignore[index]
            f"m{measure.attrib.get('number')}={duration_end(measure)}" for measure in measures if duration_end(measure) != 16
        ]
    ...
    fields = {
        ...
        "atlas-source-repeat-ending": "Repeat bars and 1/2 endings are visible on the immutable page; the retained OMR does not encode a complete repeat/ending witness",
        ...
        "atlas-lyrics": "omitted; the retained OMR contains no directly aligned lyric underlay",
        ...
        "atlas-blocker": "The source page visibly prints BETHEL. C.M., F Minor, William Cowper 1772, four vocal parts, lyrics, and repeat/ending notation. The retained OMR exports only 10 measures per part while Audiveris logged 12 raw measures, has empty measures and duration failures, and omits lyrics, shapes, and repeat/ending structure. No notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly prints BETHEL. C.M., F Minor, William Cowper 1772, four vocal parts, lyric underlay, and repeat/ending notation.",
        "The retained source OMR exports 10 measure elements per part with event counts P1=29, P2=46, P3=30, P4=19 and six empty measures; Audiveris reports 12 raw measures, so complete source measure topology is not established.",
        "The retained OMR has duration failures against 4/4 in the exact per-part measures recorded in durationFailuresAgainst4_4 and has no rests, lyrics, four-shape tags, or complete repeat/ending semantics.",
        "The existing queue metadata says Ab major, but direct inspection of the immutable page says F minor; the derivative records the source-visible key/mode and does not trust the stale queue field.",
        "No authorized same-title structured candidate exists, so no alternate tune or edition was used to fill missing notation.",
        "No separate retained work/source-images/2025 duplicate matching 27t Bethel exists; the immutable work/omr/27t-bethel/source.jpg is the canonical local visual witness.",
    ]
    audit = {
        "queueId": "sh2025/27t",
        ...
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=27t",
            ...
            "directObservations": {
                ...
                "parts": 4,
                "lyricsVisible": True,
                "stanzaLinesVisible": 6,
                "repeatBarsVisible": True,
                "endingsVisible": True,
                "sourceMeasureCountStatus": "not safely adjudicated from the dense one-system raster; OMR exports 10 per part and Audiveris reports 12 raw measures",
            },
            "retainedSourceImageMissing": {"expectedGlob": "work/source-images/2025/*27t*bethel*.jpg", "status": "not-found"},
        },
        "inputOmr": {"path": "work/omr/27t-bethel/source.mxl", "sha256": source_hash, "status": "retained-source-scan-omr"},
        "candidateWitness": {"status": "none-authorized", "sameTitleStructuredCandidate": False},
        "correctedDraft": {
            "path": "work/omr/autonomous-transcriptions/2025/27t-bethel-source-correction-v2.mxl",
            "sha256": draft_hash,
            "summary": summary,
            ...
            "corrections": [
                "source F-minor key and explicit minor mode",
        ...
        "blockingReason": "Autonomous promotion is blocked by unresolved source measure topology, incomplete OMR events and durations, absent direct lyrics/shapes/repeat evidence, stale conflicting queue key metadata, and no authorized structured witness. The derivative preserves detected events and adds source metadata/derived shapes without fabrication.",
    ...
    print(json.dumps({"record": "sh2025/27t", "status": audit["comparisonStatus"], "sourceImageSha256": image_hash, "inputOmrSha256": source_hash, "correctedDraftSha256": draft_hash, "summary": summary, "audit": str(AUDIT.relative_to(ROOT))}, ensure_ascii=False, sort_keys=True))
    return 0
