#!/usr/bin/env python3
"""Create a source-derived, fail-closed Unrath derivative."""
...
SOURCE_IMAGE = ROOT / "work/omr/395-unrath/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/395-unrath-2868a8dbda.jpg"
SOURCE = ROOT / "work/omr/395-unrath/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/395-unrath-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/395-unrath-source-correction-v2-comparison.json"

# E-flat major four-shape spelling: E-flat=fa, F=sol, G=la, A-flat=fa, B-flat=sol, C=la, D=mi.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    for part in parts:
        ...
        summary["durationFailuresAgainst4_4"][part_id] = [f"m{number}={end}" for number, end in ends.items() if end != 16]  # type: ignore[index]
    ...
    fields = {
        "atlas-queue-id": "sh2025/395", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false",
        ...
        "atlas-source-key": "E-flat major", "atlas-source-mode": "major", "atlas-source-time-signature": "4/4", "atlas-source-meter": "Long Meter (8,8,8,8)",
        ...
        "atlas-shape-encoding": "derived four-shape spelling from retained OMR pitch steps and source-visible E-flat-major key; not source-verified per event", "atlas-lyrics": "omitted; source lyrics are visible but retained OMR has no directly aligned lyric underlay",
        ...
        "atlas-blocker": "The immutable page visibly prints UNRATH. L.M., E-flat major, 4/4, Isaac Watts 1719, Keillor Mose 2021, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar. The retained source OMR exports 17 measures per part but Audiveris reports 19 raw measures across two systems; its duration ends fail the 4/4 target in 59 of 68 measures, with 10 empty measures, no lyrics, no source-confirmed shapes, and no complete repeat/ending semantics. The watermark intersects central notation. No unsupported notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly establishes Unrath L.M., E-flat major, 4/4, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar.",
        "The retained source OMR exports 17 measures per part and 178 note events (174 pitched and 4 rests), but Audiveris reports 19 raw measures; 59 of 68 exported measures fail the 4/4 duration audit and 10 measures are empty, so event timing and topology are not proven source-faithful.",
        "The retained source OMR has no lyrics, no four-shape notehead tags, and no complete source repeat/ending semantics. The derivative adds observed E-flat-major/4/4 metadata and derived shapes without rewriting uncertain events.",
    ...
    audit = {
        "queueId": "sh2025/395", "edition": "Sacred Harp, 2025 Edition", "songNo": "395", "title": "Unrath", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=395", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/300-399/395-Unrath/395.jpg", "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": image_hash, "immutable": True, "directObservations": {"header": "UNRATH. L.M.", "composer": "Isaac Watts, 1719", "arranger": "Keillor Mose, 2021", "key": "E-flat major", "mode": "major", "timeSignature": "4/4", "meter": "Long Meter (8,8,8,8)", "parts": 4, "measuresByPart": {"P1": 17, "P2": 17, "P3": 17, "P4": 17}, "lyricsVisible": True, "repeatBarsVisible": True, "endingsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True}, "retainedSourceImageDuplicate": {"path": str(RETAINED_IMAGE.relative_to(ROOT)), "sha256": retained_hash, "immutable": True, "geometryMatchesRequestedSource": True, "byteEqualToRequestedSource": False}},
        ...
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": source_events == corrected_events, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "sourceBarlines": source_barlines, "correctedBarlines": corrected_barlines, "corrections": ["source E-flat-major key and explicit major mode", "source 4/4 time signature", "four-shape noteheads derived for every retained pitched event", "source lyric/repeat/watermark visibility recorded in provenance", "lyrics and uncertain repeat/ending semantics intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": image_hash, "retainedDuplicatePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedDuplicateSha256": retained_hash, "method": "full-resolution visual inspection of requested immutable Unrath scan and retained duplicate plus retained source MXL and Audiveris audit; no alternate witness used to fill source events", "blockingFindings": blocking},
        "blockingFindings": blocking, "blockingReason": "Autonomous promotion is blocked by the 17-versus-19 measure topology discrepancy, 59 duration failures in the exported measures, 10 empty measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-obscured central systems, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
