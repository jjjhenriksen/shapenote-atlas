#!/usr/bin/env python3
"""Reconcile Sleepers Awake's retained scan OMR into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/570-sleepers-awake/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/570-sleepers-awake-705d9e8737.jpg"
RAW_OMR = ROOT / "work/omr/570-sleepers-awake/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/570-sleepers-awake-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/570-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, raw_hash: str) -> bytes:
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/570",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies SLEEPERS AWAKE! C.M., E major, 4/4, Isaac Watts (1707), Raymond C. Hamrick (2007), four vocal parts, and source-visible lyrics. Audiveris reports three systems of 11, 7, and 11 raw measures; the page visibly includes repeat bars with numbered first/second endings, terminal double bars, and a diagonal DO NOT COPY watermark across lower notation.",
        "The retained MXL exports 28 measures per part while Audiveris reports 29 raw measures. It contains 186 pitched events, 6 rest events, 31 empty exported measures, and 91 of 112 exported part-measures fail the source 4/4 duration target at divisions=6. The source event, duration, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics, repeat/ending elements, or per-note four-shape encoding. The E-major key and 4/4 meter are source observations only; adding those attributes does not prove the underlying OMR events, printed shapes, lyric syllables, or repeat topology. No unsupported event, lyric, repeat, ending, or obscured material was fabricated.",
        "No authorized exact-edition structured Sleepers Awake witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, endings, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/570", "edition": "Sacred Harp, 2025 Edition", "songNo": "570", "title": "Sleepers Awake",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=570", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/570-Sleepers-Awake/570.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "SLEEPERS AWAKE! C.M.", "composer": "Isaac Watts, 1707", "arranger": "Raymond C. Hamrick, 2007", "key": "E major", "mode": "major", "timeSignature": "4/4", "meter": "Common Meter (8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 29, "sourceRawMeasuresBySystem": [11, 7, 11], "exportedMeasuresByPart": {"P1": 28, "P2": 28, "P3": 28, "P4": 28},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Sleepers Awake witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromRetainedOmr": True, "status": "review-only-not-source-verified", "corrections": ["source-observed E-major key", "source-observed 4/4 meter", "explicit autonomous-block metadata", "lyrics/repeats/endings/shapes intentionally not fabricated"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 29-versus-28 measure topology discrepancy, 91-of-112 retained-OMR 4/4 duration failures, 31 empty exported measures, absent lyrics/repeat/ending semantics/per-note shape encoding, watermark-intersected notation, differing source-image hashes, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
