#!/usr/bin/env python3
"""Create a source-derived, fail-closed Exeter derivative."""
...
SOURCE_IMAGE = ROOT / "work/omr/409-exeter/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/409-exeter-82311886f8.jpg"
SOURCE = ROOT / "work/omr/409-exeter/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/409-exeter-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/409-exeter-source-correction-v2-comparison.json"
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    fields = {
        "atlas-queue-id": "sh2025/409", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false",
        ...
        "atlas-source-key": "F major", "atlas-source-mode": "major", "atlas-source-time-signature": "4/4", "atlas-source-meter": "Common Meter Double (8,6,8,6,8,6,8,6)",
        ...
        "atlas-blocker": "The immutable page visibly prints EXETER. C.M.D., F major, 4/4, Anne Steele 1760, Lauren R. Bock 2009, four vocal parts across 22 source measures, lyrics, numbered first/second endings, and a terminal double bar. The retained source OMR exports 21 measures per part but Audiveris reports 22 raw measures across two systems; its duration ends fail the 4/4 target in 66 of 84 measures, with 17 empty measures, no lyrics, no source-confirmed shapes, and no complete repeat/ending semantics. No unsupported notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly establishes Exeter C.M.D., F major, 4/4, four vocal parts across 22 source measures, lyrics, numbered first/second endings, and a terminal double bar.",
        "The retained source OMR exports 21 measures per part and 201 note events (197 pitched and 4 rests), but Audiveris reports 22 raw measures; 66 of 84 exported measures fail the 4/4 duration audit and 17 measures are empty, so event timing and topology are not proven source-faithful.",
        "The retained source OMR has no lyrics, no four-shape notehead tags, and no complete source repeat/ending semantics. The derivative adds observed F-major/4/4 metadata and derived shapes without rewriting uncertain events.",
    ...
    audit = {
        "queueId": "sh2025/409", "edition": "Sacred Harp, 2025 Edition", "songNo": "409", "title": "Exeter", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=409", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/409-Exeter/409.jpg", "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": image_hash, "immutable": True, "directObservations": {"header": "EXETER. C.M.D.", "composer": "Anne Steele, 1760", "arranger": "Lauren R. Bock, 2009", "key": "F major", "mode": "major", "timeSignature": "4/4", "meter": "Common Meter Double (8,6,8,6,8,6,8,6)", "parts": 4, "measuresByPart": {"P1": 22, "P2": 22, "P3": 22, "P4": 22}, "sourceRawMeasuresFromAudiveris": 22, "lyricsVisible": True, "repeatBarsVisible": True, "endingsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": False}, "retainedSourceImageDuplicate": {"path": str(RETAINED_IMAGE.relative_to(ROOT)), "sha256": retained_hash, "immutable": True, "geometryMatchesRequestedSource": True, "byteEqualToRequestedSource": True}},
        ...
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": source_events == corrected_events, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "sourceBarlines": source_barlines, "correctedBarlines": corrected_barlines, "corrections": ["source F-major key and explicit major mode", "source 4/4 time signature", "four-shape noteheads derived for every retained pitched event", "source lyric/repeat visibility recorded in provenance", "lyrics and uncertain repeat/ending semantics intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": image_hash, "retainedDuplicatePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedDuplicateSha256": retained_hash, "method": "full-resolution visual inspection of requested immutable Exeter scan and retained byte-identical image plus retained source MXL and Audiveris audit; no alternate witness used to fill source events", "blockingFindings": blocking},
        "blockingFindings": blocking, "blockingReason": "Autonomous promotion is blocked by the 21-versus-22 measure topology discrepancy, 66 duration failures in the exported measures, 17 empty measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
