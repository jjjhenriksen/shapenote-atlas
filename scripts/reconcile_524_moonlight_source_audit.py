#!/usr/bin/env python3
"""Reconcile Moonlight's retained scan OMR into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/524-moonlight/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/524-moonlight-00754707c7.jpg"
RAW_OMR = ROOT / "work/omr/524-moonlight/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/524-moonlight-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/524-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, raw_hash: str) -> bytes:
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/524",
        ...
        "atlas-repeat-ending-encoding": "not encoded; source-visible repeat ending is not independently reconstructed",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies MOONLIGHT. 7s D., E minor, 4/4, Philip Doddridge (1755), Jenny Solheim (2021), four vocal parts, and source-visible lyrics. Repeat-ending barlines are visible without numbered first/second endings; a diagonal DO NOT COPY watermark intersects the middle/lower systems.",
        "Audiveris reports 19 raw measures across three systems (9, 4, and 6), while the retained MXL exports 17 measures per part. The retained score contains 231 pitched events, 7 empty exported measures, and 53 of 68 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics, repeat-ending elements, or per-note four-shape encoding. The E-minor key and 4/4 meter are source observations only; adding those attributes does not prove the underlying OMR events or printed shapes. No unsupported event, lyric, repeat, or watermark-obscured material was fabricated.",
        "No authorized exact-edition structured Moonlight witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, endings, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/524", "edition": "Sacred Harp, 2025 Edition", "songNo": "524", "title": "Moonlight",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=524", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/524-Moonlight/524.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "MOONLIGHT. 7s D.", "composer": "Philip Doddridge, 1755", "arranger": "Jenny Solheim, 2021", "key": "E minor", "mode": "minor", "timeSignature": "4/4", "meter": "Particular Meter (7s)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 19, "sourceRawMeasuresBySystem": [9, 4, 6], "exportedMeasuresByPart": {"P1": 17, "P2": 17, "P3": 17, "P4": 17},
                "sourceLyricsVisible": True, "repeatBarsVisible": True, "numberedEndingsVisible": False, "terminalDoubleBarVisible": False, "watermarkIntersectsNotation": True,
            },
        },
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Moonlight witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromRetainedOmr": True, "status": "review-only-not-source-verified", "corrections": ["source-observed E-minor key", "source-observed 4/4 meter", "explicit autonomous-block metadata", "lyrics/repeats/shapes intentionally not fabricated"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 19-versus-17 measure topology discrepancy, 53-of-68 retained-OMR 4/4 duration failures, 7 empty exported measures, absent lyrics/repeat semantics/per-note shape encoding, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
