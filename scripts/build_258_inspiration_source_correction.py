#!/usr/bin/env python3
"""Create a source-derived, fail-closed correction for Sacred Harp 258."""
...
SOURCE = ROOT / "work/omr/258-inspiration/258-inspiration.mxl"
SOURCE_IMAGE = ROOT / "work/source-transcriptions/2025/258-inspiration.jpg"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/258-inspiration-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/258-source-shape-autonomous-blocked-comparison.json"

# Four-shape spelling for A major: A=fa, B=sol, C-sharp=la, D=fa,
# E=sol, F-sharp=la, G-sharp=mi. Accidentals remain in the score.
...
def transform() -> tuple[bytes, dict[str, object], dict[str, list[dict[str, str]]]]:
    ...
    provenance = {
        ...
        "atlas-source-image": "work/source-transcriptions/2025/258-inspiration.jpg",
        ...
        "atlas-source-title-and-credits": "INSPIRATION. C.M.; Anne Steele, 1760; Jenny Solheim, 2023",
        ...
        "atlas-blocker": "The immutable page visibly prints INSPIRATION. C.M., A major, 4/4, four vocal parts, lyrics, first/second endings, and a terminal double bar. Audiveris reports 18 raw measures across two systems (10+8), while the retained OMR exports 16 measures per part with 11 empty measures, sparse events, no explicit time signature, no mode, no lyrics, and incomplete barline semantics. A diagonal DO NOT COPY watermark crosses lower-middle notation and lyrics. No notation was synthesized.",
    }
...
def main() -> int:
    ...
    with zipfile.ZipFile(SOURCE) as source, zipfile.ZipFile(OUTPUT, "w", compression=zipfile.ZIP_DEFLATED) as target:
        for info in source.infolist():
            target.writestr(info, xml if info.filename == next(item for item in source.namelist() if item.lower().endswith(".xml") and not item.lower().startswith("meta-inf/")) else source.read(info.filename))
    ...
    blocking = [
        "The immutable scan establishes A major, 4/4, four parts, lyrics, first/second endings, and a terminal double bar, but the retained OMR has no source key/mode declaration, no aligned lyrics, and incomplete repeat/barline semantics.",
        "Audiveris reports 18 raw measures across two systems (10+8), while the exported MusicXML contains 16 measures per part; the source topology is not complete.",
        "The retained OMR contains 156 note elements, 153 pitched events, 3 rests, and 11 empty measures, with duration failures against 4/4 and sparse event coverage.",
        "The source watermark intersects lower-middle notation and lyric areas; no hidden note, lyric, repeat, ending, or duration was inferred.",
        "No authorized same-title structured candidate is present; notation was not borrowed from another tune or edition.",
    ]
    audit = {
        "queueId": "sh2025/258", "edition": "Sacred Harp, 2025 Edition", "songNo": "258", "title": "Inspiration", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=258", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/200-299/258-Inspiration/258.jpg", "sourceImagePath": "work/source-transcriptions/2025/258-inspiration.jpg", "sourceImageSha256": source_image_hash, "immutable": True, "directObservations": {"header": "INSPIRATION. C.M.", "key": "A major", "mode": "major", "timeSignature": "4/4", "meter": "Common Meter (8,6,8,6)", "parts": 4, "audiverisRawMeasures": 18, "sourceMeasuresBySystem": [10, 8], "sourceLyricsVisible": True, "sourceRepeatEnding": "first/second endings and terminal double bar visible", "watermarkAffectedRegions": "lower-middle notation and lyric areas"}},
        "inputOmr": {"path": "work/omr/258-inspiration/258-inspiration.mxl", "sha256": source_hash, "status": "retained-source-scan-omr"},
        "correctedDraft": {"path": "work/omr/autonomous-transcriptions/2025/258-inspiration-source-correction-v2.mxl", "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": True, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "corrections": ["source A-major key and explicit major mode", "source 4/4 time signature", "four-shape noteheads added to every retained pitched event", "provenance and fail-closed status fields added", "lyrics, repeats, endings, and missing notation intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": "work/source-transcriptions/2025/258-inspiration.jpg", "sourceScanSha256": source_image_hash, "method": "full-resolution direct source-image inspection plus XML event, duration, topology, and provenance audit", "blockingFindings": blocking},
        "blockingReason": "Autonomous promotion is blocked because the retained OMR is two measures short of the source topology, sparse across 11 empty measures, rhythmically unresolved, and lacks source-aligned lyrics/repeat semantics. The derivative preserves detected events and adds source metadata/shapes without inventing notation.",
    ...
    print(json.dumps({"record": "sh2025/258", "status": audit["comparisonStatus"], "sourceImageSha256": source_image_hash, "inputOmrSha256": source_hash, "correctedDraftSha256": draft_hash, "summary": summary, "audit": str(AUDIT.relative_to(ROOT))}, ensure_ascii=False, sort_keys=True))
    return 0
