#!/usr/bin/env python3
"""Create the missing source-derived, fail-closed draft for Sacred Harp 312b."""
...
SOURCE = ROOT / "work/omr/312b-restoration/source.mxl"
SOURCE_IMAGE = ROOT / "work/omr/312b-restoration/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/312b-restoration-04c164e0af.jpg"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/312b-autonomous-blocked.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/312b-restoration-autonomous-comparison.json"
...
def main() -> int:
    ...
    fields = {
        "atlas-queue-id": "sh2025/312b", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false", "atlas-source-image": "work/omr/312b-restoration/source.jpg", "atlas-source-image-sha256": sha256(SOURCE_IMAGE), "atlas-retained-source-copy": "work/source-images/2025/312b-restoration-04c164e0af.jpg", "atlas-retained-source-copy-sha256": sha256(RETAINED_IMAGE), "atlas-source-key": "A minor", "atlas-source-mode": "minor", "atlas-source-time-signature": "4/4", "atlas-source-meter": "Particular Meter (8s & 7s.)", "atlas-source-title-and-credits": "RESTORATION. 8s & 7s.; Robert Robinson, 1758; The Southern Harmony, 1835; The Hesperian Harp, 1848", "atlas-shape-encoding": "derived four-shape spelling from retained OMR pitch steps and source-visible A-minor key; not source-verified per event", "atlas-lyrics": "omitted; source lyrics are visible but retained OMR has no directly aligned lyric underlay", "atlas-provenance-policy": "immutable 2025 source image is authoritative; no authorized same-title structured candidate exists; this OMR derivative is evidence only", "atlas-blocker": "The immutable page visibly prints RESTORATION. 8s & 7s., A minor, 4/4, four vocal parts, lyrics, sectional bars, and terminal bars. The retained OMR contains six measures per part, but its barline partition is not proven against the denser source geometry and its durations are sparse. A diagonal DO NOT COPY watermark intersects middle-system notes. No notation was synthesized.",
    }
    ...
    blocking = ["The source page establishes A minor, 4/4, four parts, visible lyrics, sectional bars, and terminal bars, but the OMR has no aligned lyrics or source-confirmed per-note shapes.", "The retained OMR has six measures per part while the source scan shows denser barline geometry; its durations fail against 4/4 in the recorded measure groups, so event-level completion is unproven.", "The diagonal watermark intersects middle-system notes; no obscured note, lyric, repeat, ending, duration, or shape was inferred.", "No authorized same-title structured candidate was available, so no alternate tune or edition was used."]
    audit = json.loads(AUDIT.read_text(encoding="utf-8"))
    audit["correctedDraft"] = {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": True, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "corrections": ["source A-minor key and explicit minor mode", "source 4/4 time signature", "four-shape noteheads added to every retained pitched event", "canonical and distinct retained source-image provenance recorded", "lyrics, repeats, endings, missing events, and uncertain durations intentionally omitted"]}
    audit["blockingFindings"] = blocking
    audit["comparisonEvidence"]["blockingFindings"] = blocking
    audit["blockingReason"] = "Autonomous promotion is blocked because the source/OMR barline partition is unresolved, duration grouping is incomplete against 4/4, lyrics and complete structural semantics are absent, and watermark-intersected events are not proven. The derivative preserves detected events and adds source metadata/shapes without fabrication."
    AUDIT.write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"record": "sh2025/312b", "status": audit["comparisonStatus"], "sourceImageSha256": sha256(SOURCE_IMAGE), "retainedSourceImageSha256": sha256(RETAINED_IMAGE), "inputOmrSha256": sha256(SOURCE), "correctedDraftSha256": draft_hash, "summary": summary, "audit": str(AUDIT.relative_to(ROOT))}, ensure_ascii=False, sort_keys=True))
    return 0
