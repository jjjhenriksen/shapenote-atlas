#!/usr/bin/env python3
"""Reconcile Woodlands' retained shape draft into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/507-woodlands/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/507-woodlands-b6b9a375ac.jpg"
RAW_OMR = ROOT / "work/omr/507-woodlands/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-507-woodlands-b6b9a375ac/work__source-images__2025__507-woodlands-b6b9a375ac.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/507-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/507-woodlands-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/507-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    for part in children(root, "part"):
        ...
        # The scan prints F-sharp minor: three sharps.
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/507",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies WOODLANDS. C.M.D., F-sharp minor, 4/4, Anne Steele (1760), Rachel W. Hall (2024), four vocal parts, and source-visible lyrics. An internal repeat is visible, the page ends with a terminal double bar, and a diagonal DO NOT COPY watermark intersects the middle and lower systems.",
        "Audiveris reports 22 raw measures (13 in the first system and 9 in the second), while the retained source MXL and normalized-v2 export 21 measures per part. The normalized-v2 working score contains 231 pitched events, 18 empty exported measures, and 63 of 84 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 231 four-shape noteheads are derived from OMR pitch steps plus the observed F-sharp-minor key, not independently verified against every printed notehead. The requested immutable scan and retained catalog image share geometry but have different hashes; neither was silently substituted for the other. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "The available Steele and Wallace same-title witnesses are explicitly rejected cross-edition/source mismatches (A minor Steele C.M. and F major Wallace C.M. versus the 2025 F-sharp-minor Woodlands C.M.D.). No authorized exact-edition structured Woodlands witness was available, so those witnesses were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/507", "edition": "Sacred Harp, 2025 Edition", "songNo": "507", "title": "Woodlands",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=507", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/500-599/507-Woodlands/507.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "WOODLANDS. C.M.D.", "composer": "Anne Steele, 1760", "arranger": "Rachel W. Hall, 2024", "key": "F# minor", "mode": "minor", "timeSignature": "4/4", "meter": "Common Meter Double (8,6,8,6,8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 22, "sourceRawMeasuresBySystem": [13, 9], "exportedMeasuresByPart": {"P1": 21, "P2": 21, "P3": 21, "P4": 21},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Woodlands witness was available; same-title Steele and Wallace witnesses were rejected as cross-edition/source mismatches and not used."},
        ...
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": source_hash, "retainedImagePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedImageSha256": retained_hash, "rawOmrSha256": raw_hash, "workingOmrSha256": working_hash, "method": "full-resolution visual inspection of immutable scan plus structural/event/duration audit of raw and normalized-v2 OMR; rejected alternate witnesses retained separately and not used", "blockingFindings": blocking},
        "blockingFindings": blocking,
        "blockingReason": "Autonomous promotion is blocked by the 22-versus-21 measure topology discrepancy, 63-of-84 normalized-v2 4/4 duration failures, 18 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat semantics, watermark-intersected notation, differing source-image hashes, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
        ...
        "nextAction": "autonomous-promotion-blocked-by-incomplete-source-event-witness-and-source-image-provenance-discrepancy; retain-review-derivative-only",
        "policy": "Immutable 2025 source images remain authoritative. OMR and rejected alternate witnesses are evidence only and cannot authorize promotion without direct event-level, rhythm, lyric, repeat, mode, meter, and shape proof.",
