#!/usr/bin/env python3
"""Create a source-derived, fail-closed Night Song derivative."""
...
SOURCE_IMAGE = ROOT / "work/omr/264-night-song/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/264-night-song-39b9fd24de.jpg"
SOURCE = ROOT / "work/omr/264-night-song/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/264-night-song-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/264-night-song-source-correction-v2-comparison.json"

# F-sharp minor uses the relative A-major four-shape spelling.
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    fields = {
        "atlas-queue-id": "sh2025/264", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false",
        ...
        "atlas-source-key": "F-sharp minor", "atlas-source-mode": "minor", "atlas-source-time-signature": "4/4", "atlas-source-meter": "Long Meter Double (8,8,8,8,8,8,8,8)",
        ...
        "atlas-shape-encoding": "derived four-shape spelling from retained OMR pitch steps and source-visible F-sharp-minor key; not source-verified per event", "atlas-lyrics": "omitted; source lyrics are visible but retained OMR has no directly aligned lyric underlay",
        "atlas-provenance-policy": "immutable 2025 scan is authoritative; 264 Night Song is kept distinct from 264b; the retained source-image duplicate is preserved; this OMR derivative is evidence only",
        "atlas-blocker": "The immutable page visibly prints NIGHT SONG. L.M.D., F-sharp minor, 4/4, Isaac Watts 1719, Allison Blake Steel 2009, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar. The retained source OMR exports 27 measures per part but Audiveris reports 28 raw measures across three systems; its duration ends fail the 4/4 target in 78 of 108 measures, with no lyrics, no source-confirmed shapes, and no complete repeat/ending semantics. The watermark intersects lower systems. No unsupported notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly establishes Night Song L.M.D., F-sharp minor, 4/4, four vocal parts, lyrics, numbered first/second endings, and a terminal double bar.",
        "The retained source OMR exports 27 measures per part and 367 note events (364 pitched and 3 rests), but Audiveris reports 28 raw measures; 78 of 108 exported measures fail the 4/4 duration audit, so event timing and topology are not proven source-faithful.",
        "The retained source OMR has no lyrics, no four-shape notehead tags, and no complete source repeat/ending semantics. The derivative adds observed F-sharp-minor/4/4 metadata and derived shapes without rewriting uncertain events.",
        ...
        "The source page’s diagonal DO NOT COPY watermark crosses lower-system notation and lyric regions; obscured material was not inferred.",
        "This is sh2025/264 Night Song only; 264b artifacts were not inspected or modified.",
    ]
    audit = {
        "queueId": "sh2025/264", "edition": "Sacred Harp, 2025 Edition", "songNo": "264", "title": "Night Song", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=264", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/200-299/264-Night-Song/264.jpg", "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": image_hash, "immutable": True, "directObservations": {"header": "NIGHT SONG. L.M.D.", "composer": "Isaac Watts, 1719", "arranger": "Allison Blake Steel, 2009", "key": "F-sharp minor", "mode": "minor", "timeSignature": "4/4", "meter": "Long Meter Double (8,8,8,8,8,8,8,8)", "parts": 4, "measuresByPart": {"P1": 27, "P2": 27, "P3": 27, "P4": 27}, "lyricsVisible": True, "repeatBarsVisible": True, "endingsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True}, "retainedSourceImageDuplicate": {"path": str(RETAINED_IMAGE.relative_to(ROOT)), "sha256": retained_hash, "immutable": True, "geometryMatchesRequestedSource": True, "byteEqualToRequestedSource": True}},
        "inputOmr": {"path": str(SOURCE.relative_to(ROOT)), "sha256": source_hash, "status": "retained-source-scan-omr", "summary": input_summary},
        "candidateWitness": {"available": False, "candidateRole": "No authorized same-title structured witness was available; alternate editions and 264b were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": source_events == corrected_events, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "sourceBarlines": source_barlines, "correctedBarlines": corrected_barlines, "corrections": ["source F-sharp-minor key and explicit minor mode", "source 4/4 time signature", "four-shape noteheads derived for every retained pitched event", "source lyric/repeat/watermark visibility recorded in provenance", "lyrics and uncertain repeat/ending semantics intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": image_hash, "retainedDuplicatePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedDuplicateSha256": retained_hash, "method": "full-resolution visual inspection of requested immutable Night Song scan and retained duplicate plus retained source MXL and Audiveris audit; 264b was not used", "blockingFindings": blocking},
        "blockingFindings": blocking, "blockingReason": "Autonomous promotion is blocked by the 27-versus-28 measure topology discrepancy, 78 duration failures in the exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-obscured lower systems, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
        "nextAction": "autonomous-promotion-blocked-by-incomplete-source-event-witness-and-unresolved-topology; retain-corrected-draft-only", "policy": "Immutable 2025 Night Song scan remains authoritative. 264b is a separate record. OMR-derived events and shape tags are evidence only and cannot authorize corpus promotion without direct source proof.", "generatedAt": datetime.now(timezone.utc).isoformat(),
    }
