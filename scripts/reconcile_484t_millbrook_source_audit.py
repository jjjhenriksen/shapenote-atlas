#!/usr/bin/env python3
"""Reconcile Millbrook's retained shape draft into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/484t-millbrook/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-transcriptions/2025/484t/484t.jpg"
RAW_OMR = ROOT / "work/omr/484t-millbrook/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-484t/work__source-transcriptions__2025__484t__484t.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/484t-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/484t-millbrook-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/484t-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    for part in children(root, "part"):
        ...
        # The scan prints A Major; A is three sharps.
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/484t",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies MILLBROOK. L.M., A major, 3/4, Charles Wesley (1745), Neely Bruce (1989), four vocal parts, three source-visible verses, and a terminal double bar; the diagonal DO NOT COPY watermark intersects the middle systems. No numbered ending repeat is printed.",
        "Audiveris reports 13 raw measures in one system, matching the retained source MXL, while normalized-v2 exports 12 measures per part. The normalized-v2 working score contains 70 pitched events, 16 empty exported measures, and 38 of 48 exported part-measures fail the 3/4 duration target at divisions=1. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 70 four-shape noteheads are derived from OMR pitch steps plus the observed A-major key, not independently verified against every printed notehead. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "No authorized exact-edition structured Millbrook witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/484t", "edition": "Sacred Harp, 2025 Edition", "songNo": "484t", "title": "Millbrook",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=484t", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/484t-Millbrook/484t.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "MILLBROOK. L.M.", "composer": "Charles Wesley, 1745", "arranger": "Neely Bruce, 1989", "key": "A major", "mode": "major", "timeSignature": "3/4", "meter": "Long Meter (8,8,8,8)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 13, "sourceRawMeasuresBySystem": [13], "exportedMeasuresByPart": {"P1": 12, "P2": 12, "P3": 12, "P4": 12},
                "sourceLyricsVisible": True, "sourceVerseCount": 3, "repeatBarsVisible": False, "numberedEndingsVisible": False, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True,
            },
        },
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Millbrook witness was available; alternate editions were not used."},
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": draft, "eventStreamPreservedFromPriorReviewDraft": True, "status": "review-only-not-source-verified", "corrections": ["source-observed A-major mode", "source-observed 3/4 meter", "explicit autonomous-block metadata", "preserved derived four-shape tags without claiming per-note verification", "lyrics intentionally omitted"]},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 13-versus-12 measure topology discrepancy, 38-of-48 normalized-v2 3/4 duration failures, 16 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete event coverage, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
