#!/usr/bin/env python3
"""Create a source-derived, fail-closed Man's Redemption derivative."""
...
SOURCE_IMAGE = ROOT / "work/omr/322-man-s-redemption/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/322-man-s-redemption-54080e2713.jpg"
SOURCE = ROOT / "work/omr/322-man-s-redemption/source.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/322-mans-redemption-source-correction-v2.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/322-mans-redemption-source-correction-v2-comparison.json"
...
def transform() -> tuple[str, bytes, dict[str, object], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]], dict[str, list[dict[str, str]]]]:
    ...
    fields = {
        "atlas-queue-id": "sh2025/322", "atlas-transcription-status": "autonomously-blocked", "atlas-review-status": "autonomously-blocked-source-derived-draft", "atlas-safe-to-promote": "false",
        ...
        "atlas-source-key": "G major", "atlas-source-mode": "major", "atlas-source-time-signature": "2/4", "atlas-source-meter": "9s & 7s.",
        "atlas-source-repeat-ending": "The source visibly has a sectional double bar and terminal bars; the retained OMR does not encode complete source repeat/ending semantics, so none was fabricated.",
        ...
        "atlas-blocker": "The immutable page visibly prints MAN'S REDEMPTION. 9s & 7s., G major, 2/4, Battle's Collection 1814, S.M. Brown 1870, Alto S.M. Denson 1911, four vocal parts, lyrics, sectional structure, and terminal bars. The retained source OMR exports 23 measures per part and Audiveris reports 23 raw measures across two systems, but its duration ends fail the 2/4 target in 42 of 92 measures, with no lyrics, no source-confirmed shapes, and no complete repeat/ending semantics. The watermark intersects central notation. No unsupported notation was synthesized.",
    }
...
def main() -> int:
    ...
    blocking = [
        "The immutable scan visibly establishes Man's Redemption, 9s & 7s., G major, 2/4, four vocal parts, lyrics, sectional structure, and terminal bars.",
        "The retained source OMR exports 23 measures per part and 168 note events (all pitched), but its duration ends fail the 2/4 target in 42 of 92 measures; Audiveris reports 23 raw measures, so event timing and complete source alignment are not proven.",
        "The retained source OMR has no lyrics, no four-shape notehead tags, and no complete source repeat/ending semantics. The derivative adds observed G-major/2/4 metadata and derived shapes without rewriting uncertain events.",
        ...
        "The source page's diagonal DO NOT COPY watermark crosses central notation and lyric regions; obscured material was not inferred.",
        "This is sh2025/322 Man's Redemption only; unrelated 322 artifacts were not modified.",
    ]
    audit = {
        "queueId": "sh2025/322", "edition": "Sacred Harp, 2025 Edition", "songNo": "322", "title": "Man's Redemption", "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {"sourcePageUrl": "https://fasola.org/indexes/2025/?p=322", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/300-399/322-Mans-Redemption/322-Mans-Redemption.jpg", "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": image_hash, "immutable": True, "directObservations": {"header": "MAN'S REDEMPTION. 9s & 7s.", "credits": "Battle's Collection, 1814; S.M. Brown, 1870; Alto S.M. Denson, 1911", "key": "G major", "mode": "major", "timeSignature": "2/4", "meter": "9s & 7s.", "parts": 4, "measuresByPart": {"P1": 23, "P2": 23, "P3": 23, "P4": 23}, "lyricsVisible": True, "repeatBarsVisible": True, "endingsVisible": True, "terminalDoubleBarVisible": True, "watermarkIntersectsNotation": True}, "retainedSourceImageDuplicate": {"path": str(RETAINED_IMAGE.relative_to(ROOT)), "sha256": retained_hash, "immutable": True, "geometryMatchesRequestedSource": True, "byteEqualToRequestedSource": False}},
        ...
        "correctedDraft": {"path": str(OUTPUT.relative_to(ROOT)), "sha256": draft_hash, "summary": summary, "eventStreamPreservedFromInput": source_events == corrected_events, "sourceEventSignature": source_events, "correctedEventSignature": corrected_events, "sourceBarlines": source_barlines, "correctedBarlines": corrected_barlines, "corrections": ["source G-major key and explicit major mode", "source 2/4 time signature", "four-shape noteheads derived for every retained pitched event", "source lyric/repeat/watermark visibility recorded in provenance", "lyrics and uncertain repeat/ending semantics intentionally omitted"]},
        "comparisonEvidence": {"sourceScanInspected": True, "sourceScanPath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceScanSha256": image_hash, "retainedDuplicatePath": str(RETAINED_IMAGE.relative_to(ROOT)), "retainedDuplicateSha256": retained_hash, "method": "full-resolution visual inspection of requested immutable Man's Redemption scan and retained duplicate plus retained source MXL and Audiveris audit; no alternate witness used to fill source events", "blockingFindings": blocking},
        "blockingFindings": blocking, "blockingReason": "Autonomous promotion is blocked by incomplete source event timing, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-obscured central systems, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
