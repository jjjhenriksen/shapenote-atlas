#!/usr/bin/env python3
"""Reconcile Radiance's retained shape draft into an autonomous block."""
...
SOURCE_IMAGE = ROOT / "work/omr/488-radiance/source.jpg"
RETAINED_IMAGE = ROOT / "work/source-images/2025/488-radiance-b2b9ec715d.jpg"
RAW_OMR = ROOT / "work/omr/488-radiance/source.mxl"
WORKING_OMR = ROOT / "work/omr/cleaned-normalized-v2-488-radiance-b2b9ec715d/work__source-images__2025__488-radiance-b2b9ec715d.mxl"
PRIOR_DRAFT = ROOT / "work/omr/source-shape-review-drafts/2025/488-source-shape-review.mxl"
OUTPUT = ROOT / "work/omr/autonomous-transcriptions/2025/488-radiance-source-correction.mxl"
AUDIT = ROOT / "work/source-transcriptions/2025/488-source-shape-autonomous-blocked-comparison.json"
...
def update_xml(xml_bytes: bytes, source_hash: str, working_hash: str) -> bytes:
    ...
    fields = {
        "atlas-review-queue-id": "sh2025/488",
...
def main() -> int:
    ...
    blocking = [
        "The immutable page visibly identifies RADIANCE. C.M.D., F major, 4/4, Anne Steele (1760), Angharad Davis (2014), four vocal parts, source-visible lyrics, and first/second endings with a terminal double bar; a diagonal DO NOT COPY watermark intersects the middle systems.",
        "Audiveris reports 19 raw measures (7 in each outer system and 5 in the middle system), while the retained source MXL exports 18 measures per part and normalized-v2 exports 20 measures per part. The normalized-v2 working score contains 243 pitched events, 13 empty exported measures, and 68 of 80 exported part-measures fail the 4/4 duration target at divisions=2. The source event stream, durations, and measure topology are therefore not proven complete.",
        "The retained OMR and corrected derivative contain no lyrics or repeat/ending elements. Their 243 four-shape noteheads are derived from OMR pitch steps plus the observed F-major key, not independently verified against every printed notehead. No unsupported event, lyric, repeat, or obscured watermark-intersected material was fabricated.",
        "No authorized exact-edition structured Radiance witness was available. Alternate-edition records were not used to fill notes, durations, lyrics, repeats, or shapes.",
    ]
    audit = {
        "queueId": "sh2025/488", "edition": "Sacred Harp, 2025 Edition", "songNo": "488", "title": "Radiance",
        "comparisonStatus": "autonomously-blocked", "autonomousDecision": "blocked", "safeToPromote": False, "humanReviewRequired": False,
        "sourceAuthority": {
            "sourcePageUrl": "https://fasola.org/indexes/2025/?p=488", "sourceImageUrl": "https://sacredharpbremen.org/wp-content/uploads/songs/400-499/488-Radiance/488.jpg",
            "sourceImagePath": str(SOURCE_IMAGE.relative_to(ROOT)), "sourceImageSha256": source_hash, "immutable": True,
            "directObservations": {
                "header": "RADIANCE. C.M.D.", "composer": "Anne Steele, 1760", "arranger": "Angharad Davis, 2014", "key": "F major", "mode": "major", "timeSignature": "4/4", "meter": "Common Meter Double (8,6,8,6,8,6,8,6)", "parts": 4,
                "sourceRawMeasuresFromAudiveris": 19, "sourceRawMeasuresBySystem": [7, 5, 7], "exportedMeasuresByPart": {"P1": 20, "P2": 20, "P3": 20, "P4": 20},
        ...
        "candidateWitness": {"available": False, "candidateRole": "No authorized exact-edition structured Radiance witness was available; alternate editions were not used."},
        ...
        "blockingReason": "Autonomous promotion is blocked by the 19-versus-18/20 measure topology discrepancy, 68-of-80 normalized-v2 4/4 duration failures, 13 empty exported measures, absent lyrics and source-confirmed per-note shapes, incomplete repeat/ending semantics, watermark-intersected notation, and lack of an authorized exact-edition structured witness. The corrected derivative remains review-only.",
