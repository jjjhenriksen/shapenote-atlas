...
def main():
    data = json.loads((PACKAGE / 'socialharp-page-map-supplement-v6.json').read_text())
    ...
    rows = data['records']
    assert [row['record_id'] for row in rows] == [row['record_id'] for row in remaining[:5]]
    assert len({row['record_id'] for row in rows}) == 5
    ...
    for row, original in zip(rows, remaining):
        ...
        # Only this directly reviewed punctuation difference is accepted.
        expected_heading = {'socialharp 145 — ROLL, JORDAN': 'ROLL JORDAN'}.get(original['record_id'], original['title'])
        assert row['literal_heading'].removesuffix('.').replace('’', "'") == expected_heading
