...
def check_structure(root, prior, evidence):
    ...
    for tag in ['lyric','notehead','mode','repeat','ending','barline','backup','forward','chord']:
    ...
    assert len(observations) == 4*(evidence['completeMeasuresPerPart']-1)
    counts = {}
    ...
    for part, old_part, v1 in zip(parts, old_parts, evidence['voiceObservations']):
        ...
        assert len(old) == 1
        assert canonical(measures[0]) == canonical(old[0]), 'v1 first-bar XML changed'
        ...
        for number, measure in enumerate(measures, 1):
            ...
            if number > 1:
                ...
                assert obs['xRange'] == evidence['coordinateSystem']['firstSystemBarlinesX'][number-1:number+1]
                assert all(n.find('rest') is None for n in notes), 'unobserved new rest'
    ...
    assert sum(counts.values()) - 4 == evidence['addedPitchedNotes']
    expected = []
    for curve in evidence['curveObservations']:
        for part_id in curve['parts']:
            ...
            if curve['kind'] == 'tie':
                ...
                assert xml_pitch(a) != xml_pitch(b), 'slur unexpectedly joins equal pitches'
    ...
    return dict(parts=4, pitchedNotesByPart=counts, pitchedNotes=sum(counts.values()),
                addedPitchedNotes=sum(counts.values())-4,
                ...
                exactV1PrefixPreserved=True, ties=expected.count('tie'), slurs=expected.count('slur'))
...
def check(evidence):
    ...
    mutations = {
        ...
        'pitch drift': lambda r: setattr(r.find("part/measure[@number='2']/note/pitch/octave"),'text','8'),
        'duration drift': lambda r: setattr(r.find("part/measure[@number='2']/note/duration"),'text','99'),
        'missing source curve': lambda r: r.find('.//notations').clear(),
    }
...
if __name__ == '__main__':
    results = []
    for path in sorted(BASE.glob('*-evidence-v2.json')):
